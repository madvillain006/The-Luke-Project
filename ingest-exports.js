...
const EXPORTS_DIR  = path.join(__dirname, 'discord-exports', 'processed');
const HISTORY_FILE = path.join(__dirname, 'discord-history.jsonl');
const BOBBY_FILE   = path.join(__dirname, 'bobby-context.jsonl');
const PROGRESS_LOG = path.join(__dirname, 'logs', 'strategy-progress.log');
...
function runIngestion() {
  ...
  fs.appendFileSync(
    PROGRESS_LOG,
    `[${new Date().toISOString()}] [phase1.6] ingest rewired to new parsers\n`
  );
}
