...
const { ROOT: LUKE_ROOT, events } = require("./paths");

const ACTION_LOG_FILE    = events.actionLog;
const TOOL_FAILURES_FILE = events.toolFailures;
const TOOL_CALLS_FILE    = events.toolCalls;
const SCREEN_ACTIONS_FILE = events.screenActions;
...
async function _handleAction(action) {
  if (action.action === "open") {
    ...
    runPython(["click", String(x), String(y)]);
    ...
    if (!/^(1|true|yes|on)$/i.test(String(process.env.LUKE_ALLOW_ANTHROPIC_VISION || ""))) {
      return "VISION DISABLED: LUKE_ALLOW_ANTHROPIC_VISION is not enabled.";
    }
    const Anthropic = require("@anthropic-ai/sdk");
    const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
    const img = runPython(["screenshot"]);
    ...
    const result = spawnSync(action.command, { shell: true, windowsHide: true, encoding: "utf8", timeout: timeoutMs, cwd });
    const out = {
    ...
    const result = spawnSync(cmd, { shell: true, windowsHide: true, encoding: "utf8", timeout: 10000, cwd });
