...
const ACTION_LOG_FILE    = path.join(JARVIS_ROOT, "action-log.jsonl");
const TOOL_FAILURES_FILE = path.join(JARVIS_ROOT, "tool-failures.jsonl");
const TOOL_CALLS_FILE    = path.join(JARVIS_ROOT, "tool-calls.jsonl");
const SCREEN_ACTIONS_FILE = path.join(JARVIS_ROOT, "screen-actions.jsonl");
...
function runPython(cmd) {
  return execSync("python desktop.py " + cmd, { cwd: JARVIS_ROOT }).toString().trim();
}
...
async function _handleAction(action) {
  if (action.action === "open") {
    ...
    const cwd = action.cwd ? path.resolve(JARVIS_ROOT, action.cwd) : JARVIS_ROOT;
    ...
    try {
      const rawPath = path.isAbsolute(action.path) ? action.path : path.join(JARVIS_ROOT, action.path);
      const filePath = path.resolve(rawPath);
      if (!filePath.startsWith(JARVIS_ROOT)) {
        logToolFailure({ op: "read", path: action.path, error: "path_traversal_blocked" });
        return "READ BLOCKED: path outside Jarvis root";
      }
    ...
    try {
      const dirPath = action.path
        ? (path.isAbsolute(action.path) ? action.path : path.join(JARVIS_ROOT, action.path))
        : JARVIS_ROOT;
