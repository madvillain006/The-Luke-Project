...
function closedContracts(plan, config = {}) {
  const cfg = normalizeConfig(config);
  if (cfg.contractPlan === 'split_tp1_tp2') return 2;
  return Number(plan?.contracts || 1);
}

function buildOutcome(outcome, bar, points, plan, config = {}) {
  const cfg = normalizeConfig(config);
  const contracts = closedContracts(plan, cfg);
...
function evaluateOutcome(bars, startIndex, plan, config = {}) {
  const fill = accountingEntry(plan, config);
