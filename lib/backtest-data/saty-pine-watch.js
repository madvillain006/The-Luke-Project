...
function evaluateOutcome(bars, startIndex, plan, config = {}) {
  for (let i = startIndex + 1; i < bars.length; i += 1) {
    ...
    if (hitStop && hitTp1) {
      const points = mixedPoints(plan.stop, plan.entry, plan.tp1, config);
      return {
        outcome: points > 0 ? 'mixed_target_first' : points === 0 ? 'mixed_zero' : 'mixed_stop_first',
        outcome_timestamp: bar.timestamp,
        points,
        dollars: round2(points * normalizeConfig(config).pnlPointValue),
      };
    }
    if (hitStop) {
      const points = plan.stop - plan.entry;
      return {
        outcome: 'stop_first',
        outcome_timestamp: bar.timestamp,
        points,
    ...
    if (hitTp2) {
      const points = plan.tp2 - plan.entry;
      return {
        outcome: 'tp2_first',
        outcome_timestamp: bar.timestamp,
        points,
    ...
    if (hitTp1) {
      const points = plan.tp1 - plan.entry;
      return {
        outcome: 'tp1_first',
        outcome_timestamp: bar.timestamp,
        points,
...
function evaluatePineWatchSession({ bars, satyLevels, config = {} }) {
  ...
  for (let i = 1; i < sortedBars.length; i += 1) {
    ...
    for (const cluster of clusters) {
      ...
      if (paperHere && (!paperPlan || level > paperPlan.level)) paperPlan = { ...plan, trigger: impulseReclaimHere ? 'impulse_reclaim' : 'reclaim_hold' };
    ...
    const longCandidateEvent = paperSetupNow && !openTrade && (paperEdgeEvent || paperRetestReentry);
...
function runSatyPineWatchBacktest({ bars, referenceField = 'close', dates = null, config = {} }) {
  ...
  for (const date of targetDates) {
    ...
    const result = evaluatePineWatchSession({ bars: sessionBars, satyLevels: saty, config });
  ...
  return {
    ...
    formula: {
      pine_file: 'tradingview/luke-level-reclaim-watch.pine',
