...
const { inferInstrument } = require('./confluence');
...
const INSTRUMENT_SPECS = {
  ...
  SPY: { tick: 0.01, dpt: 1.00,  defaultTarget: 3,  defaultStop: 1.5 },
...
function calculateBracket(signal, confluenceZones, currentPrice) {
  ...
  // Instrument filter — prevent cross-instrument zone bleed (P0 fix)
  const signalInstrument = inferInstrument(entry, ticker);
  if (!signalInstrument) {
    return { error: 'Cannot infer instrument for ticker', ticker, strike: entry };
  }

  const validZones = (confluenceZones || []).filter(z => {
    if (!z.instrument && !z.ticker) return true;
    const zoneInstrument = z.instrument || inferInstrument(z.level, z.ticker);
    return zoneInstrument === signalInstrument;
  });

  // No matching zones — fall through to default bracket (spec.defaultTarget / spec.defaultStop)

  const hm = validZones.filter(
  ...
  if (validZones.length === 0) {
    result.default_bracket = true;
    if (!result.flag) {
      result.flag = 'warning';
      result.flag_message = 'No confluence zones — default bracket only';
    }
  }
