...
function buildDailyBrief(input = {}) {
  ...
  const radarBrief = input.radarBrief || input.radar || null;
  ...
  const radarItems = (radarBrief?.ideas_to_verify || []).slice(0, 3).map(item => ({
    title: item.title,
    source: item.source_label || item.source_type || 'Radar',
    url: null,
    published_at: null,
    why_it_matters: (item.evidence || []).join(' / ') || 'Radar item needs human review before it becomes trusted context.',
  }));
  if (radarBrief) {
    sections.unshift({
      id: 'radar-review',
      label: 'Radar review',
      category: 'radar',
      status: radarItems.length ? 'review' : 'clear',
      items: radarItems,
      empty_note: radarItems.length ? null : radarBrief.empty_note || 'Radar has no review item right now.',
    });
  }
  ...
  return {
    ...
    radar_status: radarBrief?.summary_line || 'not_checked',
    sections,
