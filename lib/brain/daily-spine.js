...
function emptyRadarSummary(now = new Date()) {
  return {
    ok: true,
    label: 'Radar',
    generated_at: now.toISOString(),
    counts: { total: 0, fresh_24h: 0, review: 0 },
    review_queue: [],
    recent: [],
    source_counts: {},
    source_type_counts: {},
    ...
    source_health: [],
    theme_counts: {},
    summary_line: 'ready / no items',
  };
}

function readRadarSummary(paths, now = new Date()) {
  if (!paths?.events?.radarIngest || !paths?.snapshots?.radarState) return emptyRadarSummary(now);
  try {
    return buildRadarSnapshot(paths, now, { writeSnapshot: false });
  } catch {
    return {
      ...emptyRadarSummary(now),
      ok: false,
      summary_line: 'radar unavailable',
    };
  }
}
...
function buildDailySpine(options = {}) {
  ...
  const radar = options.radar || readRadarSummary(paths, now);
  const radarReviewCount = radar.counts?.review || 0;
  const radarReminders = (radar.review_queue || [])
    .filter(item => item.review_priority === 'reminder' || (item.themes || []).includes('reminder'))
    .slice(0, 5);
  ...
  const checklist = [
    ...
    },
    {
      id: 'radar-review',
      label: 'Radar review',
      status: radarReviewCount ? 'open' : 'done',
      detail: radarReviewCount
        ? `${radarReviewCount} Radar item(s) need review before they become Daily or Trading context.`
        : 'Radar has no pending review item.',
    },
    {
  ...
  const radarReminderActions = radarReminders
    .map(item => `Radar reminder: ${item.title || item.raw_text_preview || item.source_label || item.id}`)
    .slice(0, 5);
  const nextActions = blockers.length
    ? [...radarReminderActions, ...blockers]
    : (radarReminderActions.length ? radarReminderActions : ['No immediate daily blocker detected.']);
  ...
  return {
    ...
    pipeline: {
      ...
      radar_fresh_24h: radar.counts?.fresh_24h || 0,
      radar_review_items: radarReviewCount,
      radar_reminders: radarReminders.length,
    },
    radar: {
      ok: radar.ok !== false,
      summary_line: radar.summary_line,
      counts: radar.counts,
      review_queue: (radar.review_queue || []).slice(0, 5),
      reminders: radarReminders,
      source_type_counts: radar.source_type_counts || {},
      review_state_counts: radar.review_state_counts || {},
      source_health: radar.source_health || [],
    },
    ...
    things_to_know: [
      ...
      'Luke is the local companion system; trading is one separate human-gated module.',
      'Daily briefs use live RSS feeds when available; X sources need configured feed bridges or API access.',
      'Radar review items must be accepted or archived before they become trusted operating context.',
    ],
    briefs: {
      morning: {
