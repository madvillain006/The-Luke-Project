...
const SUBAGENTS = [
  {
    ...
    owns: ['Gemini first path', 'Groq/Llama fallback path', 'DeepSeek fallback path', 'local Ollama fallback path', 'Anthropic smart-task escalation', 'feature gating'],
...
const DEFAULT_PROVIDER_ORDER = [
  ...
  },
  {
    id: 'groq',
    priority: 2,
    label: 'Groq hosted Llama/open models',
    mode: 'fast-hosted-open-model-fallback',
    requirement: 'GROQ_API_KEY or config/fallback-config.json groq_key',
    lane: 'remote/API',
    data_leaves_machine: true,
  },
  {
    id: 'deepseek',
    priority: 3,
    label: 'DeepSeek hosted API',
    mode: 'low-cost-hosted-reasoning-fallback',
    requirement: 'DEEPSEEK_API_KEY or config/fallback-config.json deepseek_key',
    lane: 'remote/API',
    data_leaves_machine: true,
  },
  {
  ...
  },
  {
    ...
    mode: 'paid-smart-escalation-only',
...
function getFallbackConfig(paths) {
  const configFile = paths.config?.fallback || (paths === defaultPaths ? defaultPaths.config.fallback : null);
...
function getProviderReadiness(paths, env = process.env) {
  ...
  const ollamaModel = env.OLLAMA_MODEL || fallbackConfig.ollama_model || null;
  return DEFAULT_PROVIDER_ORDER.map(provider => {
    ...
    if (provider.id === 'claude') {
      ...
      status = configured ? 'configured' : 'missing_key';
      model = env.GEMINI_MODELS || env.GEMINI_MODEL || fallbackConfig.gemini_models || fallbackConfig.gemini_model || 'gemini-2.5-flash,gemini-2.5-flash-lite';
    } else if (provider.id === 'groq') {
      configured = Boolean(env.GROQ_API_KEY || fallbackConfig.groq_key);
      model = env.GROQ_MODEL || fallbackConfig.groq_model || 'llama-3.3-70b-versatile';
      status = configured ? 'configured' : 'missing_key';
    } else if (provider.id === 'deepseek') {
      configured = Boolean(env.DEEPSEEK_API_KEY || fallbackConfig.deepseek_key);
      model = env.DEEPSEEK_MODEL || fallbackConfig.deepseek_model || 'deepseek-v4-flash';
      status = configured ? 'configured' : 'missing_key';
    } else if (provider.id === 'ollama') {
      configured = Boolean(env.OLLAMA_HOST || fallbackConfig.ollama_host);
      model = ollamaModel;
      status = configured
        ? ollamaModel ? 'host_and_model_configured_unverified' : 'missing_model'
        : 'missing_host';
    }
    return { ...provider, configured, status, model };
  });
}
...
function buildDeveloperStackSpine(options = {}) {
  ...
  const ollama = providerOrder.find(provider => provider.id === 'ollama');
  const blockers = [
    ...blocked.flatMap(agent => agent.blockers),
    ...(!configured.length ? ['no developer LLM providers configured'] : []),
    ...(ollama?.configured && !ollama.model ? ['OLLAMA_MODEL not configured for local/private lane'] : []),
  ];
  ...
  return {
    ...
    status: blocked.length ? 'blocked' : !configured.length ? 'degraded' : blockers.length ? 'attention' : 'building',
    blockers,
    mission: 'Run developer-agent work through a cost-aware provider ladder: Gemini/Groq/DeepSeek/Ollama first, Anthropic only for smart-task escalation.',
    provider_order: providerOrder,
    local_only_truth: 'Only the Ollama/local-model lane is local-only. Claude and Gemini calls leave the machine.',
    health_checks: {
      claude_key: providerOrder.find(provider => provider.id === 'claude')?.status || 'missing',
      gemini_key: providerOrder.find(provider => provider.id === 'gemini')?.status || 'missing',
      groq_key: providerOrder.find(provider => provider.id === 'groq')?.status || 'missing',
      deepseek_key: providerOrder.find(provider => provider.id === 'deepseek')?.status || 'missing',
      ollama_host: ollama?.configured ? 'configured' : 'missing_host',
      ollama_model: ollama?.model || null,
      ollama_runtime: options.ollamaHealth || 'not_checked',
    },
    subagents,
    pipeline: {
      ...
      remote_api_providers: configured.filter(provider => provider.lane === 'remote/API').map(provider => provider.id),
      local_private_providers: configured.filter(provider => provider.lane === 'local/private' && provider.model).map(provider => provider.id),
      recent: events.slice(-12),
    },
    setup_plan: [
      'Keep free/low-cost providers first for routine and direct text routes.',
      'Use Gemini as the first configured provider once GEMINI_API_KEY or fallback config is present.',
      'Use Groq or DeepSeek hosted APIs when their keys are present.',
      'Install Ollama, pull a local open model, then use that lane for free/private long loops.',
      'Use Anthropic only when the routing policy marks the request as a smart task and the free provider ladder cannot satisfy it.',
