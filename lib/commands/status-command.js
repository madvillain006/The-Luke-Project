...
async function handleStatusCommand(message, res, deps) {
  ...
  statusLines.push(`Saty: generated from previous close levels${satyStatus ? ' (stored)' : ''}`);
  statusLines.push(`Freshness: ${formatLoadedFlag(workflowStatus.dubzLoaded, 'Dubz', workflowStatus.dubzCount)} | ${formatLoadedFlag(workflowStatus.bobbyLoaded, 'Bobby', workflowStatus.bobbyCount)} | Saty OK (generated)`);
  statusLines.push('Autonomous: recommendation-only; no autonomous staging or execution');
  const prepLine = getMorningPrepLine(apexStatus, workflowStatus);
  ...
  statusLines.push('Luke chat: active for trading ops (/status /ready /entries ES /heatmap /balance)');
  try {
    const katParts = [];
