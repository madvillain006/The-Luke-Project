...
async function handleVerdictCommand(message, res, deps) {
  ...
  const warning = missing.length > 0
    ? `Context warning: missing ${missing.join(', ')}. This is confluence-only; use /entries for PASS/trade truth.\n\n`
    : '';
  const reply = warning + renderVerdict(instruments, { currentPrices, topN, priceError });
