...
function makeEntry(input = {}, classification, now) {
  ...
  return {
    ...
    recall_reason: input.recall_reason || null,
  };
}
...
function searchCompanionMemory(input = {}, options = {}) {
  ...
  return store.entries
    .filter(entry => options.excludeRejected ? entry.status !== 'rejected' : true)
...
function buildCompanionContext(input = {}, options = {}) {
  ...
  return [
    ...
    `- Active surface: ${surface}. Luke Chat and Trading share one companion memory bin and one merged routing layer.`,
