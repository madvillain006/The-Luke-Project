...
function inferInstrument(price, tickerHint) {
  if (!price || price <= 0) return null

  if (tickerHint) {
    const t = tickerHint.toUpperCase()
    if (['ES', 'MES'].includes(t)) return 'ES'
    if (['NQ', 'MNQ'].includes(t)) return 'NQ'
    if (t === 'SPX') return 'SPX'
    if (t === 'SPY') return 'SPY'
    if (t === 'QQQ') return 'QQQ'
  }

  if (price >= 10000) return 'NQ'
  if (price >= 1000 && price < 10000) return 'ES'
  if (price >= 100 && price < 1000) return 'SPY'

  return null
}

function getToleranceByInstrument(instrument) {
  if (instrument === 'ES' || instrument === 'NQ') return 10.0;
  return 2.0;
}
...
function extractBobbyLevels(bob) {
  ...
  const results  = [];
  for (const l of (bob.king_nodes || [])) if (l > 0) results.push({ level: l, type: 'king_node', bias, hasImage, ts_ms });
  for (const l of (bob.support   || [])) if (l > 0) results.push({ level: l, type: 'support',   bias, hasImage, ts_ms });
  for (const l of (bob.resistance|| [])) if (l > 0) results.push({ level: l, type: 'resistance', bias, hasImage, ts_ms });
  return results;
}
