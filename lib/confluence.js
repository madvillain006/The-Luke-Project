...
const { getCachedPrice } = require('./live-price');

const LEVEL_FREQ_FILE = path.join(__dirname, '../data/level-frequency.json');
let _levelFreqCache = null;
let _levelFreqMtime = 0;

function loadLevelFrequency() {
  try {
    if (!fs.existsSync(LEVEL_FREQ_FILE)) return {};
    const mtime = fs.statSync(LEVEL_FREQ_FILE).mtimeMs;
    if (mtime === _levelFreqMtime && _levelFreqCache) return _levelFreqCache;
    _levelFreqCache = JSON.parse(fs.readFileSync(LEVEL_FREQ_FILE, 'utf8'));
    _levelFreqMtime = mtime;
    return _levelFreqCache;
  } catch (e) {
    console.error('[confluence] level-freq load error:', e.message);
    return {};
  }
}
...
function extractBobbyLevels(bob) {
  ...
  const results  = [];
  // Bobby exclusively covers SPX. Use live SPX ±20% when cached; always fall back to > 1000 (fail-open).
  function inferBobbyInstrument(level) {
    const lp = getCachedPrice();
    if (lp && lp.spx > 0 && level >= lp.spx * 0.80 && level <= lp.spx * 1.20) return 'SPX';
    if (level > 1000) return 'SPX';
...
function detectConfluence(ximesSignals, bobbyContext, katZones) {
  ...
  const katLevels = (Array.isArray(katZones) ? katZones : [])
    .filter(z => !z.bias_only && z.level != null)
    .map(z => ({
      level:      z.level,
      score:      z.score,
      ...
      analyst:    z.analyst,
      bias:       z.bias,
      ts_ms:      new Date(z.ts).getTime()
    }))
    .filter(k => k.instrument !== null);

  const instruments = [...new Set([
    ...xiLevels.map(x => x.instrument),
    ...bobLevels.map(b => b.instrument),
    ...katLevels.map(k => k.instrument)
  ])];
  ...
  for (const instrument of instruments) {
    ...
    const katGroup = katLevels.filter(k => k.instrument === instrument);

    const allValues = [...new Set([
      ...xiGroup.map(x => x.level),
      ...bobGroup.map(b => b.level),
      ...katGroup.map(k => k.level)
    ...
    for (const cluster of clusters) {
      ...
      const katHits = katGroup.filter(k => inZone(k.level, zoneRange));
      for (const kat of katHits) {
        score += kat.score;
        sources.push('kat:' + kat.analyst);
        if (kat.bias && kat.bias !== 'NEUTRAL') biases.push(kat.bias);
        if (kat.ts_ms > 0) timestamps.push(kat.ts_ms);
      }
  ...
  const levelFreq = loadLevelFrequency();

  for (const zone of zones) {
    if (zone.level == null) continue;
    const bucket = String(Math.round(zone.level / 5) * 5);
    const freq   = levelFreq[bucket];
    if (freq && freq.confluence_bonus > 0) {
      zone.score        += freq.confluence_bonus;
      zone.kat_bonus     = freq.confluence_bonus;
      zone.kat_analysts  = freq.analyst_count;
      zone.kat_count     = freq.count;
      zone.kat_bias      = freq.dominant_bias;
    }
  }

  // Re-sort after bonus applied
