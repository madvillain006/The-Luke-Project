...
const KATBOT_ANALYST_PASTE_RE = /\[\d{1,2}:\d{2}\s*[AP]M\]|\b(?:barrysanders329|barry\s+sanders|general\s+dollars|kaprik0rn3|kaprikorn3|el\s+jefe|mathemeatloaf7|trade-floor|barrys-breakdowns|spy-qqq-es-nq-vix|short-term|mid-long-term|heatmap-requests)\b/i;
const OPTION_CONTRACT_RE = /\b(?:[A-Za-z]{1,5}\s+)?(?:(?:\d{1,2}[/-]\d{1,2}(?:[/-]\d{2,4})?|\d+DTE)\s+)?\d{2,5}(?:\.\d{1,2})?\s*[cCpP]\b/;
const DIRECT_TRADE_PASTE_RE = /\b(?:buy(?:ing)?|bought|long|short|calls?|puts?|entry|entered|stop|target|tp\s*\d*|trim|runner|paid)\b/i;
const EXPLICIT_HEATMAP_RE = /\b(?:heatmap|heat\s+map|king\s+node|gatekeeper|air\s+pocket|spotgamma|hiro|gamma|call\s+wall|put\s+wall|trinity)\b/i;
...
function classifyPaste(text) {
  ...
  if ((OPTION_CONTRACT_RE.test(t) || (KATBOT_ANALYST_PASTE_RE.test(t.slice(0, 500)) && DIRECT_TRADE_PASTE_RE.test(t))) && !EXPLICIT_HEATMAP_RE.test(t)) {
    return { type: null };
  }

  // KatBot/SPX heatmap text: only auto-ingest when the parser found concrete levels.
  // Bias-only heatmap commentary is useful when explicitly pasted, but it is too
  // permissive for normal chat questions that mention loaded heatmap context.
  ...
  // Discord-style KatBot analyst/server paste falls through to the Stage 2
  // manual analyst capture lane, which can parse it or push back precisely.
  if (KATBOT_ANALYST_PASTE_RE.test(t.slice(0, 500))) {
    return { type: null };
  }
...
async function handlePasteAccumulate(message, image, res) {
  try {
    ...
    //  Image paste  heatmap (KatBot analyst/SPX context)
    if (image) {
    ...
    //  KatBot/SPX heatmap text
    if (classified.type === 'bobby') {
      ctx.heatmap = { source: 'katbot-heatmap-text', stored_at: new Date().toISOString() };
      saveDailyContext(ctx);
      log('daily-ctx', { event: 'katbot-heatmap-text-stored' });
    ...
    if (classified.type === 'saty_partial') {
      ...
      res.json({
        reply: [
          ...
          'Saty is generated automatically from previous close levels.',
          'Use /saty only for a deliberate manual override with all 13 levels.',
        ].join('\n'),
      });
      return true;
    }
    ...
    if (classified.type === 'unknown') {
      res.json({
        reply: [
          "I don't recognize enough to count this paste yet.",
          "If it is analyst context, include ticker/contract, direction, entry, stop or invalidation, target/update, and timestamp.",
          "Otherwise tell me what lane it belongs to:",
