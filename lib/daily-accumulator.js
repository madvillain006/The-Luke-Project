...
// â”€â”€ Context persistence â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Kat/Jefe SPX trade text patterns â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Paste classifier â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function classifyPaste(text) {
  ...
  // parseXimes is intentionally permissive for real alerts and can false-positive on
  // analyst commentary that mentions strikes or directions.
  ...
  // Kat/Jefe SPX trade text â€” must not be a RichyDubz paste
  ...
  if (SATY_HINT_RE.test(t) || (nums.length >= 5 && nums.length < 13 && !/\b(spx|es|nq|spy|qqq)\b/i.test(t))) {
...
// â”€â”€ Ack message builder â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const TYPE_LABELS = {
...
function buildAckMessage(type, missing) {
  ...
  if (missing.length === 0) return `Got it â€” ${label} stored. Checklist complete.`;
  return `Got it â€” ${label} stored. Still waiting: ${missing.join(', ')}.`;
}

// â”€â”€ res wrappers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Main accumulator hook â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function handlePasteAccumulate(message, image, res) {
  try {
    ...
    // â”€â”€ Image paste â†’ heatmap (Bobby or Jefe/Kat) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if (image) {
      ...
      if (complete && ctx.ximes) {
        ...
        const prefix = `Got it â€” heatmap image stored.\n${heatmapNote}\n\nChecklist complete â€” firing verdict:\n\n`;
    ...
    if (classified.type === null) return false; // conversational â€” fall through

    const ctx = loadDailyContext();

    // â”€â”€ Ximes live entry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if (classified.type === 'ximes_live') {
      ...
      if (complete) {
        const prefix = 'Got it â€” Ximes signal stored. Checklist complete â€” firing verdict:\n\n';
    ...
    // â”€â”€ Ximes pre-market setup (store + ack; no verdict â€” no live strike yet) â”€
    ...
    // â”€â”€ Ximes management â€” fall through to existing parseXimes intercept â”€â”€â”€â”€
    if (classified.type === 'ximes_mgmt') return false;

    // â”€â”€ Bobby heatmap text â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if (classified.type === 'bobby') {
      ...
      if (complete && ctx.ximes) {
        ...
        const heatmapNote = cap.result?.reply || 'Heatmap stored.';
        const prefix = `Got it â€” Bobby heatmap stored.\n${heatmapNote}\n\nChecklist complete â€” firing verdict:\n\n`;
        await handleSlashCommand('/alert ' + ctx.ximes.raw, prependRes(res, prefix));
      } else {
        const { missing } = checklistComplete(ctx);
        const ackMsg = buildAckMessage('heatmap', missing);
    ...
    // â”€â”€ Kat/Jefe SPX trade text (optional; store + ack only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Unknown trading-looking paste â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if (classified.type === 'dubz_partial') {
      ...
      res.json({
        reply: [
          ...
          'If this is Bobby, use /heatmap instead.',
        ].join('\n'),
      });
      return true;
    }
    ...
    if (classified.type === 'unknown') {
      res.json({
        reply: [
          ...
          "â†’ Ximes signal â€” paste as-is",
          "â†’ Bobby/Jefe heatmap text â€” paste as-is, or drop an image",
          "â†’ Kat context â€” paste as-is",
          "â†’ Something else? Tell me what this is.",
        ].join('\n'),
      });
      return true;
    }
    ...
    return false; // fail open â€” fall through to existing handling
  }
}
