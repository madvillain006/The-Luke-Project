...
function buildStoredHeatmapAnswer(message, options = {}) {
  ...
  return [
    ...
    bobby.vision_parsed
      ? 'This is stored image-derived context, not live price. Use `/verdict ES` or `/entries ES` only after Saty and live signal context are loaded.'
      : 'This is stored text-derived heatmap context, not live price. Use `/verdict ES` or `/entries ES` only after Saty and live signal context are loaded.',
  ].filter(Boolean).join('\n');
}
