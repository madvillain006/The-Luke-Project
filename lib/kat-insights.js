...
function buildKatInsights(options) {
  ...
  const latestVision = visionSignals
    .slice()
    .sort((a, b) => String(b.ts || '').localeCompare(String(a.ts || '')))
    .slice(0, 10);
  const visionCounts = {
    total: visionSignals.length,
    chart: visionSignals.filter(row => row.source_class === 'chart').length,
    heatmap: visionSignals.filter(row => row.source_class === 'heatmap').length,
    image: visionSignals.filter(row => row.source_class === 'image').length,
    parsed_levels: visionSignals.filter(row => row.parse_status === 'parsed_levels').length,
  };
  ...
  if (audit.raw.image_posts > 0 && visionSignals.length === 0) blockers.push('Analyst images are captured but no saved chart/heatmap reads exist yet.');
  if (audit.raw.image_posts > 0 && visionCounts.chart === 0) blockers.push('Chart images have not been read into saved level/bias records yet.');
  if (audit.raw.heatmap_candidates > 0 && visionCounts.heatmap === 0) blockers.push('Heatmap images have not been read into saved level/bias records yet.');
  ...
  const helpfulOutputs = [
    ...
    'Luke can consume saved chart/heatmap reads through data/kat/vision-signals.jsonl and the kat_vision websocket payload.',
    'The backtesting agent can use data/kat/derived/kat-index-replay.jsonl, data/kat/vision-signals.jsonl, and kat-spx-spy-evaluations.jsonl when it finishes its current lane.',
  ];

  const nextActions = [
    'Import more SPX/SPY market bars covering the raw Kat date range so more than 114 direct signals can be scored.',
    'Run historical image parsing for captured analyst chart and heatmap attachments; write every result to data/kat/vision-signals.jsonl by message_id and attachment_id.',
...
function formatKatInsightsForDiscord(insights) {
  ...
  const lines = [
    ...
    'Vision: ' + (insights.vision && insights.vision.counts ? insights.vision.counts.total : 0) +
      ' parsed | charts: ' + (insights.vision && insights.vision.counts ? insights.vision.counts.chart : 0) +
      ' | heatmaps: ' + (insights.vision && insights.vision.counts ? insights.vision.counts.heatmap : 0),
