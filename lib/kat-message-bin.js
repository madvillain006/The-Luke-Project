...
function uniqueLevelsFromMessages(messages) {
  const seen = new Set();
  const levels = [];
  for (const message of messages || []) {
    for (const level of message.parsed && Array.isArray(message.parsed.levels) ? message.parsed.levels : []) {
...
function buildOperatorReadouts(confluenceExamples, visionExamples) {
  const readouts = [];
  for (const example of confluenceExamples || []) {
    const levels = uniqueLevelsFromMessages(example.messages);
    const imageCount = (example.messages || []).filter(message => message.parsed && message.parsed.has_image).length;
    readouts.push({
      kind: 'confluence_readout',
      status: example.group_id && example.group_id.startsWith('index:') ? 'luke_candidate' : 'shadow_watch',
      headline: `${example.group} ${example.bias} confluence`,
      source_window: `${example.first_ts} -> ${example.last_ts}`,
      what_kat_did: [
        `Grouped ${example.signal_count} analyst signal(s) into ${example.group}.`,
        `Confirmed ${example.analysts.length} distinct analyst(s) aligned ${example.bias.toLowerCase()} inside ${example.window_minutes} minute(s).`,
        levels.length ? `Extracted level context: ${levels.join(', ')}.` : 'No numeric level was extracted from these messages.',
        imageCount ? `${imageCount} message(s) carried image evidence.` : 'No chart image evidence attached to this confluence sample.',
      ],
      luke_action: levels.length
        ? 'Push to Luke as human-gated confluence context for /entries or /verdict review.'
        : 'Keep in Luke as directional context until a level, chart read, or current level context confirms it.',
      discord_action: 'No public Discord response unless owner explicitly enables output.',
      evidence: {
        analysts: example.analysts,
        levels,
        messages: (example.messages || []).map(message => ({
          ts: message.ts,
          analyst: message.analyst,
          channel: message.channel,
          message_id: message.message_id,
          ticker: message.parsed.ticker,
          bias: message.parsed.bias,
          levels: message.parsed.levels,
          input: truncate(message.raw_input, 260),
        })),
      },
    });
  }
  ...
  for (const example of [
    ...((visionExamples && visionExamples.charts) || []),
    ...((visionExamples && visionExamples.heatmaps) || []),
  ]) {
    readouts.push({
      kind: 'vision_readout',
      status: example.parsed.levels.length ? 'luke_candidate' : 'needs_human_review',
      headline: `${example.source_class} image read: ${example.parsed.ticker || 'unknown'} ${example.parsed.bias}`,
      source_window: example.ts,
      what_kat_did: [
        `Read attached ${example.parsed.chart_type || example.source_class} image from ${example.analyst || 'unknown'}.`,
        example.parsed.levels.length ? `Extracted levels: ${example.parsed.levels.join(', ')}.` : 'No usable levels extracted yet.',
        example.parsed.heatmap_context ? 'Included heatmap node context.' : 'No heatmap node context attached.',
      ],
      luke_action: example.parsed.levels.length
        ? 'Push to Luke as chart-derived confluence context, human-gated.'
        : 'Show in Luke as a chart-read attempt only; do not treat as confluence.',
      discord_action: 'No public Discord response unless owner explicitly enables output.',
      evidence: {
        ts: example.ts,
        ...
        ticker: example.parsed.ticker,
        chart_type: example.parsed.chart_type,
        levels: example.parsed.levels,
        input: truncate(example.raw_input, 260),
      },
    });
  }

  return readouts.slice(0, 10);
}
...
function renderExamplesMarkdown(bin) {
  ...
  for (const readout of bin.operator_readouts) {
    lines.push('');
    lines.push(`### ${readout.headline}`);
    lines.push(`- Status: ${readout.status}`);
    lines.push(`- Source window: ${readout.source_window}`);
    for (const item of readout.what_kat_did) lines.push(`- Kat did: ${item}`);
    lines.push(`- Luke action: ${readout.luke_action}`);
    lines.push(`- Discord action: ${readout.discord_action}`);
  }

  if (!bin.operator_readouts.length) {
    lines.push('- No operator readout yet. Kat needs parsed signals or saved chart/heatmap reads before this section becomes useful.');
  }
...
<main>
  ...
  <p class="meta">Generated ${escapeHtml(bin.generated_at)}. Operator proof of what Katbot did with analyst input.</p>
  <h2>Operator Readouts</h2>
  ${bin.operator_readouts.map(readout => `
    <section class="example">
      <h3>${escapeHtml(readout.headline)}</h3>
      <div class="meta">${escapeHtml(readout.status)} | ${escapeHtml(readout.source_window)}</div>
      <ul>${readout.what_kat_did.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul>
      <p><b>Luke:</b> ${escapeHtml(readout.luke_action)}</p>
      <p><b>Discord:</b> ${escapeHtml(readout.discord_action)}</p>
      <pre>${escapeHtml(JSON.stringify(readout.evidence, null, 2))}</pre>
    </section>
  `).join('') || '<p>No operator readout yet. Kat needs parsed signals or saved chart/heatmap reads.</p>'}
...
function buildKatMessageBin(options = {}) {
  ...
  const bin = {
    ...
    operator_readouts: buildOperatorReadouts(confluenceExamples, visionExamples),
