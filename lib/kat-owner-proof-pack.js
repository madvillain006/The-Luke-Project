...
<main>
  ...
  <section class="card wide">
    <h2>Pipeline</h2>
    <div class="flow">
      ${['Discord message','Kat capture gate','raw-feed.jsonl','parse-kat','processed-signals.jsonl','index lane','equity/options lane','Luke trading window','owner review','Discord output gate'].map((label, i, arr) => `<span class="node">${escapeHtml(label)}</span>${i < arr.length - 1 ? '<span class="arrow">→</span>' : ''}`).join('')}
    </div>
  </section>
  <section class="grid">
    <div class="card">
    ...
    <div class="card luke">
      <h2>Luke Trading Window Preview</h2>
...
<head>
  <meta charset="utf-8">
  <title>Luke Trading Window Kat Preview</title>
  <style>
...
<body>
<main>
  <h1>Luke Trading Window: Kat Incoming Events</h1>
  <p>Internal preview only. These are Luke-side payloads, not Discord messages.</p>
