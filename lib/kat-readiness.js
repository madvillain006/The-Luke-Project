...
function recommendationFrom(checks, config) {
  ...
  const repliesEnabled = config.discord_responses_enabled === true;
  const postsEnabled = config.discord_posts_enabled === true;

  if (postsEnabled) {
    return {
      status: 'do_not_use_publicly',
      label: 'Automatic Discord posts are enabled before Phase 2 approval.',
    };
  }
  ...
  if (repliesEnabled) {
    return {
      status: 'phase_1_command_replies_live',
      label: 'Command replies are live for owner-approved Phase 1. Automatic posts remain gated.',
    };
  }
  ...
  if (warnings.length) {
    return {
      ...
      label: 'Ready for owner review before Phase 1 command replies are enabled.',
    };
  }

  return {
    status: 'ready_to_recommend_limited_enable',
    label: 'Ready to recommend limited server-owner enablement with automatic posts still gated.',
  };
}
...
function buildKatReadiness(options = {}) {
  ...
  const checks = [
    ...
    scoreCheck(audit.raw.heatmap_candidates === 0 || visionHeatmapCount > 0, 'heatmap images are read for levels', `${visionHeatmapCount} saved heatmap read(s) for ${audit.raw.heatmap_candidates} candidate(s)`, 'warning'),
    ...
    scoreCheck(true, 'Discord command replies Phase 1 state', config.discord_responses_enabled === true ? 'enabled for command-channel Phase 1 replies' : 'disabled until owner approval'),
    scoreCheck(config.discord_posts_enabled !== true, 'Discord channel posts gated off', 'magnet/confluence posts are disabled until Phase 2 approval'),
  ];
  ...
  return {
    ...
    warnings,
    owner_review_notes: [
      'Phase 1 allows summoned command replies only in configured command channels.',
      'Do not enable automatic channel posts until one clean command-reply session is reviewed.',
...
function formatKatReadinessMarkdown(readiness) {
  const lines = [
    ...
    '',
    '## Owner Notes',
    `- Discord output state: replies=${readiness.discord_output_gate.responses_enabled}, posts=${readiness.discord_output_gate.posts_enabled}`,
