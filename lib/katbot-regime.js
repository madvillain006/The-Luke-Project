...
function getKatbotRegime() {
  ...
  if (bobbyCtx.some(e => e.vix_mentioned === true)) {
    return { regime: 'RISK_OFF', selectivity: 'HIGH', max_trades_today: 1, confidence_boost: 0, reason: 'VIX mentioned in KatBot/Bobby context today' };
  }
  ...
  if (bobbyCtx.some(e => (e.raw || e.content || '').toUpperCase().includes('TRINITY') || (e.signal_type || '').toUpperCase().includes('TRINITY'))) {
    return { regime: 'RISK_ON', selectivity: 'NORMAL', max_trades_today: 3, confidence_boost: 1, reason: 'KatBot/Bobby TRINITY signal today' };
  }
...
module.exports = { getKatbotRegime };
