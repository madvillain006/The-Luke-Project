 ...
 *               source_type, source_snippet?, source_id?, timestamp? })
 *
 ...
 *
 * Returns { canonical_price, total_mentions, created_new, deduped? } on success.
...
async function recordLevel({
  ...
  // Bobby/Saty are intraday-overwrite analysts: purge all their existing mentions
  // before inserting new ones so each call replaces rather than accumulates.
  if (analyst === 'Bobby' || analyst === 'Saty') {
    for (const lvl of memory.levels) {
      lvl.mentions = lvl.mentions.filter(m => m.analyst !== analyst);
      lvl.total_mentions = lvl.mentions.length;
    }
    memory.levels = memory.levels.filter(lvl => lvl.mentions.length > 0);
  }
  ...
  if (canonical) {
    if (source_id && canonical.mentions.some(m =>
      m.analyst === analyst &&
      m.source_id === source_id &&
      m.source_type === source_type &&
      (m.significance || null) === (significance || null) &&
      (m.direction || null) === (direction || null) &&
      (m.intent || null) === (intent || null)
    )) {
      return {
    ...
    // Append-only (C6): never modify existing mentions unless source_id marks an exact duplicate
