...
const { buildContextBinsSnapshot } = require('./luke-context-bins');
const { buildRadarSnapshot } = require('./radar/ingest');
const { normalizeMarketSymbol, _internal: marketDataInternal } = require('./market-data');
...
function buildMarketDataCheck(symbols = ['ES', 'NQ', 'SPX', 'SPY', 'QQQ']) {
  const providerMap = Object.fromEntries(symbols.map(symbol => {
    const providers = marketDataInternal.providerListFor(normalizeMarketSymbol(symbol));
    return [symbol, providers];
  }));
  const weak = Object.entries(providerMap).filter(([, providers]) => providers.length < 2);
  if (weak.length > 0) {
    return {
      check: makeCheck(
        'market-data',
        'Market Data',
        'red',
        `Provider ladder weak: ${weak.map(([symbol, providers]) => `${symbol}:${providers.length}`).join(', ')}.`,
        'trading'
      ),
      providerMap,
    };
  ...
  return {
    check: makeCheck(
      ...
      'Market Data',
      'green',
      `Provider ladders ready: ${Object.entries(providerMap).map(([symbol, providers]) => `${symbol} ${providers.join('>')}`).join('; ')}.`,
      'trading'
    ),
    providerMap,
  };
...
function buildLukeOperatorCheck(options = {}) {
  ...
  let contextBinsSnapshot = null;
  let radarSnapshot = null;
  let marketDataProviders = null;
  ...
  checks.push(makeCheck(
    ...
    'Luke Chat',
    'green',
    'Luke Chat and Trading are one conversation layer with routed context behind it.'
  ));

  try {
    contextBinsSnapshot = options.contextBinsSnapshot || buildContextBinsSnapshot({ paths, limit: 4, recentLimit: 8 });
    checks.push(makeCheck(
      'context-bins',
      'Context Bins',
      'green',
      contextBinsSnapshot.summary_line || 'rolling context bins ready'
    ));
  } catch (err) {
    checks.push(makeCheck('context-bins', 'Context Bins', 'red', err.message));
    nextActions.push('Repair rolling context bins before relying on Luke as the merged chat surface.');
  }
  ...
  checks.push(makeCheck(
    ...
    'Trading Boundary',
    'green',
    'Trading messages are routed to the trading lane without leaving the merged Luke conversation.'
  ));

  const marketData = options.marketDataCheck || buildMarketDataCheck(options.marketDataSymbols);
  marketDataProviders = marketData.providerMap;
  checks.push(marketData.check);
  if (marketData.check.status === 'red') {
    nextActions.push('Repair market-data provider ladders before using Luke for trading review.');
  }
  ...
  const radarDecisions = radarSnapshot?.review_state_counts || {};
  const radarDecisionCount = (radarDecisions.accepted || 0) + (radarDecisions.contradicted || 0);
  const radarOperatorLine = radarSnapshot
    ? `${radarSnapshot.summary_line}${radarDecisionCount > 0 ? ` | ${radarDecisions.accepted || 0} accepted, ${radarDecisions.contradicted || 0} contradicted` : ''}`
    : 'unavailable';
  ...
  return {
    ...
    context_bins: contextBinsSnapshot ? {
      summary_line: contextBinsSnapshot.summary_line,
      active_bins: Object.values(contextBinsSnapshot.bins || {}).filter(bin => bin.count > 0).map(bin => bin.id),
      recent_count: contextBinsSnapshot.recent?.length || 0,
    } : null,
    market_data: {
      provider_ladders: marketDataProviders || {},
      minimum_hookups_ok: Object.values(marketDataProviders || {}).every(providers => providers.length >= 2),
    },
    radar: radarSnapshot ? {
      summary_line: radarSnapshot.summary_line,
      counts: radarSnapshot.counts || {},
      review_state_counts: radarSnapshot.review_state_counts || {},
    } : null,
    ...
    operator_lines: [
      ...
      `Bins: ${contextBinsSnapshot?.summary_line || 'unavailable'}`,
      `Market data: ${checks.find(check => check.id === 'market-data')?.detail || 'unavailable'}`,
      `Radar: ${radarOperatorLine}`,
