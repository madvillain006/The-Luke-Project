...
function providerPriority(provider, providers) {
  const index = providers.indexOf(provider);
  return index >= 0 ? index + 1 : 99;
}

function safeProviderError(err) {
  if (!err) return 'provider_failed';
  const message = err.message || String(err);
  return message.replace(/(?:api[_-]?key|token|secret|password)=?[^&\s]*/ig, 'SECRET_REDACTED');
}

function withAttemptMetadata(result, providers, errors, fallbackUsed = false) {
  return {
    ...result,
    ...
    minimum_hookups_ok: providers.length >= 2,
  };
}
...
async function getMarketPrice(symbol, options = {}) {
  ...
  for (const provider of providers) {
    let result;
    try {
      result = await callProvider(provider, info, options);
    } catch (err) {
      result = makeUnknownResult(info, safeProviderError(err), provider, providerPriority(provider, providers));
    }
    if (successful(result)) {
      const resolved = withAttemptMetadata(result, providers, errors, Object.keys(errors).length > 0);
      if (cacheEnabled) setCachedMarketPrice(info.symbol, providers, resolved, options.cacheTtlMs);
      return resolved;
    }
  ...
  const unknown = withAttemptMetadata(
    makeUnknownResult(info, Object.entries(errors).map(([provider, error]) => `${provider}:${error}`).join('; ') || 'no_provider_available'),
    providers,
    errors,
    providers.length > 0
  );
...
async function getMarketSnapshot(symbols, options = {}) {
  ...
  const pairs = await Promise.all(list.map(async symbol => {
    try {
      return [symbol, await getMarketPrice(symbol, options)];
    } catch (err) {
      const info = normalizeMarketSymbol(symbol);
      return [symbol, withAttemptMetadata(makeUnknownResult(info, safeProviderError(err)), [], {}, false)];
    }
  }));
