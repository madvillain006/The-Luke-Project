...
function getETComponents(now = new Date()) {
  ...
  return { h, m, etMins: h * 60 + m, weekday, date: `${year}-${month}-${day}` };
}
...
function isMarketDate(date, { holidays = [] } = {}) {
  return WEEKDAYS.includes(weekdayForDate(date)) && !isHoliday(date, holidays);
}

function previousMarketDay(date, options = {}) {
  let cursor = addDays(date, -1);
  for (let i = 0; i < 14; i += 1) {
    if (isMarketDate(cursor, options)) return cursor;
    cursor = addDays(cursor, -1);
  }
  throw new Error(`No previous market day found before ${date}`);
}

function marketSessionDate(now = new Date(), options = {}) {
  const { date, etMins } = getETComponents(now);
  if (isMarketDate(date, options)) return date;
  if (weekdayForDate(date) === 'Sun' && etMins >= FUTURES_OPEN_MINS) return addDays(date, 1);
  return previousMarketDay(date, options);
}
...
module.exports = {
  ...
  previousMarketDay,
  _internal: { addDays, getETComponents, weekdayForDate },
};
