...
function bridgeLongsAccepted(options = {}) {
  if (options.allowLong === true) return true;
  return String(process.env.LUKE_NINJA_BRIDGE_ACCEPT_LONGS || "").toLowerCase() === "true";
}
...
function latencyMs(fromIso, toIso) {
...
function normalizeLukeLongPayload(input, now = new Date()) {
  ...
  const rawBarTime = firstDefined(source.bar_time, source.barTime, source.bar_timestamp, source.barTimestamp);
  const barTime = rawBarTime === undefined ? null : normalizeTimestamp(rawBarTime, now);
...
function saveLukeBridgeCommand(input, options = {}) {
  ...
  if (signal.type === "LUKE_LONG" && !bridgeLongsAccepted(options)) {
    throw new Error("TradingView Ninja bridge LONGs are disabled; use Ninja-native execution or set LUKE_NINJA_BRIDGE_ACCEPT_LONGS=true for an intentional fallback test");
  }
  ...
  const event = {
    ...
    bar_time: signal.bar_time || null,
    received_at: signal.received_at,
    source_to_luke_ms: latencyMs(signal.created_at, signal.received_at),
  };
