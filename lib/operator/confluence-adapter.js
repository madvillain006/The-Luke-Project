...
const { getMarketSnapshot, normalizeMarketSymbol } = require('../market-data');
  ...
  try {
    if (getLivePriceFn === getLivePrice && getMarketSnapshotFn) {
      ...
      for (const instrument of instruments) {
        ...
        if (!isUsableDecisionPrice(row)) {
          warnings.push(Number.isFinite(row?.price)
            ? `${instrument} market price not trusted`
            : `${instrument} market price UNKNOWN`);
        }
      }
    ...
    const warnings = [];
    for (const instrument of instruments) {
      const row = makeUnknownResult(normalizeMarketSymbol(instrument), err.message);
      currentPrices[instrument] = null;
      marketData[instrument] = summarizeMarketData(row);
      warnings.push(`${instrument} market price UNKNOWN`);
    }
    return { currentPrices, warnings, marketData };
  }
}
