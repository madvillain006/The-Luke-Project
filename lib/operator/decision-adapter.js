...
function marketDataFromLegacyLive(instrument, livePrice) {
  ...
  return {
    ...
    error: Number.isFinite(resolved) ? null : 'live price unavailable',
    raw: livePrice || null,
  };
}
...
function decisionBlockedByAdapter(decision, warnings, currentPrice = null) {
  if (warnings.some(warning => /market price unavailable|market price stale|live price unavailable/i.test(warning))) {
    const side = decision.action === 'LONG' || decision.action === 'SHORT' ? decision.action : decision.side || null;
    return {
      ...decision,
      action: 'PASS',
      side,
      reason: `WAIT - market price unavailable | ${decision.reason}`,
      adapter_blocker: 'market_price_unavailable',
    };
  }
...
async function resolveCurrentPrice({ instrument, currentPrice, getLivePriceFn, getMarketPriceFn }) {
  ...
  try {
    ...
    return {
      ...
      warning: usable ? null : (Number.isFinite(marketData?.price) ? 'market price stale or delayed' : 'market price unavailable'),
      marketData,
    };
  } catch (err) {
    return { currentPrice: null, warning: `market price unavailable: ${err.message}`, marketData: null };
  }
}
...
async function buildDecisionResponse({
  ...
  const decision = decisionBlockedByAdapter(spineDecision, warnings, price.currentPrice);
