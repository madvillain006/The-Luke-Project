...
function fileSummary(key, readResult) {
  const isSatyState = key === 'saty';
  const isAbsentState = readResult.status === 'missing';
  if (isSatyState && isAbsentState) {
    return {
      status: 'generated_from_previous_close',
...
function buildIngestionStatus({ now = new Date(), files = FILES } = {}) {
  ...
  if (!freshness.dubz.loaded) blockers.push('Dubz structural levels missing');
  ...
  return {
    ...
    state_files: Object.fromEntries(
      Object.entries(reads).map(([key, result]) => [key, fileSummary(key, result)])
    ),
