...
const { isMarketOpen, isFuturesMarketOpen, isGoodTradingTime, minsUntilOpen, minsUntilFuturesOpen } = require('../market-hours');
...
function isFuturesInstrument(instrument) {
  return ['ES', 'MES', 'NQ', 'MNQ'].includes(String(instrument || '').toUpperCase());
}

function marketState(instrument = 'ES') {
  const useFutures = isFuturesInstrument(instrument);
  const market = useFutures ? isFuturesMarketOpen() : isMarketOpen();
  const tradingTime = isGoodTradingTime();
  return {
    market,
    cash_market: isMarketOpen(),
    futures_market: isFuturesMarketOpen(),
    trading_time: tradingTime,
    minutes_until_open: market.open ? 0 : (useFutures ? minsUntilFuturesOpen() : minsUntilOpen()),
  };
}
...
async function buildOperatorStatus({
  ...
  return {
    ...
    blockers,
    mode,
    market_state: marketState(instrument),
    ...
    autonomous: {
      staged_only: true,
      recommendation_only: true,
      wording: 'Autonomous only sends recommendations to Luke chat; no autonomous staging or execution.',
