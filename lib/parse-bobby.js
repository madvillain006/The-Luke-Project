...
async function parseBobbyImage(base64Image) {
  ...
  try {
    response = await client.messages.create({
      ...
      max_tokens: 600,
      system: `This is a Bobby heatmap from OWLS bobby-spx-coms. Identify: king nodes (magnet zones), air pockets (fast move zones), trinity signals (SPX+SPY+QQQ alignment), purple deflection floors (support), walls (resistance). Return JSON only, no explanation. Format: {"king_nodes":[],"air_pockets":[],"trinity":false,"walls":[],"floors":[],"overall_bias":"BULLISH|BEARISH|NEUTRAL"}`,
      messages: [{
        role: 'user',
        content: [{
          type: 'image',
  ...
  if (king_nodes.length + support.length + resistance.length + air_pockets.length === 0) return null;
