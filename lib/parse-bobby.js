...
const { buildBobbyHeatmapSourceId } = require('./bobby-heatmap-idempotency');
const { buildHeatseekerReferencePrompt } = require('./heatseeker-reference');

function anthropicVisionAllowed() {
  return /^(1|true|yes|on)$/i.test(String(process.env.LUKE_ALLOW_ANTHROPIC_VISION || ''));
}
  ...
  for (const pat of patterns) {
    ...
    while ((m = re.exec(text)) !== null) {
      ...
      for (const pm of window.matchAll(/\$?\b([\d,]{3,9}(?:\.\d+)?)\b/g)) {
        ...
        if (isValidTextPrice(n) && !found.includes(n)) found.push(n);
      }
    }
  }
  return found;
...
function isLikelyCalendarYear(n) {
  return Number.isInteger(n) && n >= 1900 && n <= 2099;
}

function isValidTextPrice(n) {
  return !isNaN(n) && n > 100 && !isLikelyCalendarYear(n);
}
...
function parseBobby(messageText) {
  ...
  if (king_nodes.length + support.length + resistance.length === 0) {
    ...
    for (const pm of text.matchAll(/\$?([\d,]{3,7}(?:\.\d+)?)/g)) {
      const n = parseFloat(pm[1].replace(/,/g, ''));
      if (isValidTextPrice(n) && !plain.includes(n)) plain.push(n);
    }
    if (plain.length === 0) {
...
async function parseBobbyImage(base64Image, livePrices = null) {
  ...
  if (!anthropicVisionAllowed()) {
    return { parse_status: 'failed', error: 'Vision/OCR disabled until LUKE_ALLOW_ANTHROPIC_VISION is enabled.' };
  }

  // Strip data URL prefix if present
  const raw64 = base64Image.replace(/^data:image\/[^;]+;base64,/, '');
  const source_id = buildBobbyHeatmapSourceId({ image: raw64 });

  const Anthropic = require('@anthropic-ai/sdk');
  const client = new Anthropic();

  // G2: use passed livePrices or fetch internally. getLivePrice() never throws;
  // provider failures return a structured UNKNOWN snapshot.
...
BOBBY MESSAGE RULES:
${JSON.stringify(BOBBY_HEATMAP_RULES, null, 2)}
      ...
      for (const { price, significance, direction, crossSourceConfirmed } of entries) {
        try {
          await recordLevel({ analyst: 'bobby', instrument, price, significance, direction,
            intent: null, source_type: sourceType, source_snippet: snippet, source_id: sourceId, timestamp,
