...
function pricesNear(text, patterns) {
  ...
  for (const pat of patterns) {
    ...
    while ((m = re.exec(text)) !== null) {
      ...
      for (const pm of window.matchAll(/\$?\b([\d,]{3,7}(?:\.\d+)?)\b/g)) {
...
async function appendBobbyToMemory(result) {
  ...
  if (result.panels && result.panels.length > 0) {
    for (const panel of result.panels) {
      ...
      const entries = [
        ...(panel.king_nodes || []).map(price => ({ price, significance: 'key',     direction: null })),
        ...(panel.support    || []).map(price => ({ price, significance: 'unclear', direction: 'support' })),
        ...(panel.resistance || []).map(price => ({ price, significance: 'unclear', direction: 'resistance' })),
      ];
