...
async function parseBobbyImage(base64Image) {
  ...
  const rulesText = JSON.stringify(BOBBY_HEATMAP_RULES, null, 2);
  ...
  try {
    response = await client.messages.create({
      ...
      max_tokens: 512,
      system: `You are reading a Bobby SPX heatmap. Use these rules:\n${rulesText}\nExtract: king nodes (prices), support levels (prices), resistance levels (prices), bias (BULLISH/BEARISH/NEUTRAL).\nReturn JSON only. No explanation. Format: {"king_nodes":[],"support":[],"resistance":[],"bias":"NEUTRAL"}`,
      messages: [{
        role: 'user',
        content: [{
          type: 'image',
  ...
  const king_nodes = (parsed.king_nodes || []).filter(n => typeof n === 'number');
  const support    = (parsed.support || []).filter(n => typeof n === 'number');
  const resistance = (parsed.resistance || []).filter(n => typeof n === 'number');

  if (king_nodes.length + support.length + resistance.length === 0) return null;
