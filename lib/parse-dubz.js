...
function buildDubzStatus(state) {
  if (!state) {
    return [
      'WARNING No Dubz structural levels loaded.',
  ...
  if (state.date !== today) {
    const count = Object.values(state.instruments || {})
      .reduce((sum, bucket) => sum + ((bucket && bucket.levels) ? bucket.levels.length : 0), 0);
    return [
      `Dubz structural levels loaded from ${state.date || 'unknown date'} (${count} levels, updated ${age}m ago).`,
      `Today is ${today} ET. Structural levels carry forward until manually replaced or deleted.`,
      'Same-day callouts, if any, must be judged separately and expire same day.',
    ].join('\n');
  }
  const SEP   = '--------------------';
  ...
  for (const [instr, data] of Object.entries(state.instruments)) {
    if (!data.levels.length) { lines.push(`${instr}: none`); continue; }
    const parts = data.levels.map(l => {
      ...
      const src    = l.source === 'image' ? ' image' : '';
  ...
  if (state.conflicts.length > 0) {
    lines.push(`WARNING ${state.conflicts.length} conflict(s):`);
