...
const TICKER_RE = /\$?([a-zA-Z]{1,5})(?:\s|$)/g;
...
function isNoise(content, hasAttachments) {
  ...
  if (content.length < 8 && !/\d/.test(content) && !/[a-zA-Z]{2,}/.test(content)) return true;
  if (!hasAttachments) {
    const hasTicker = /\$?[a-zA-Z]{2,5}\b/.test(content);
...
function extractTicker(content) {
  ...
  const futuresTagMatch = content.match(/(?:^|[^a-zA-Z0-9_])[$#]?((?:ES|NQ|MES|MNQ|SPX|SPY|QQQ|NDX))(?:_F)?\b/i);
  if (futuresTagMatch) return futuresTagMatch[1].toUpperCase();

  // $TICKER format first (El Jefe style)
  const dollarMatch = content.match(/\$([a-zA-Z]{1,5})\b/);
  if (dollarMatch) return dollarMatch[1].toUpperCase();

  // TICKER TIMEFRAME format (KapriK0rn3 style: "NVDA 1D.")
  const tfTickerMatch = content.match(/^([a-zA-Z]{1,5})\s+(?:1[mMdDwW]|[0-9]+[mMhH]|daily|weekly)/);
  if (tfTickerMatch) return tfTickerMatch[1].toUpperCase();
  ...
  const startMatch = content.match(/^([a-zA-Z]{2,5})\b/);
  if (startMatch) {
    const ticker = startMatch[1].toUpperCase();
    if (ticker !== 'IHS' && ticker !== 'ATH') return ticker;
  }
