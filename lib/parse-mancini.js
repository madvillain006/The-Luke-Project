...
function inferTargetDirection(context) {
  ...
  if (/\bfailed breakdown\b/i.test(text)) return 'resistance';
  if (/\bfailed breakout\b/i.test(text)) return 'support';
  return /\b(short|sell|fails?|breakdown)\b/i.test(text) ? 'support' : 'resistance';
}
...
function addClauseTargets(levelMap, parse_errors, instrument, text, clauseText, source_snippet, kind) {
  ...
  prices.forEach((price, idx) => {
    ...
    pushOrUpgrade(levelMap, buildLevel(price, source_snippet, {
      ...
      direction:    inferTargetDirection(`${source_snippet} ${clauseText}`),
      intent,
    }));
  });
}

function addClauseLevels(levelMap, parse_errors, instrument, clauseText, source_snippet, fieldsForIndex) {
  const prices = extractPrices(clauseText).filter(price => validatePrice(instrument, price, parse_errors, source_snippet));
  prices.forEach((price, idx) => {
    pushOrUpgrade(levelMap, buildLevel(price, source_snippet, fieldsForIndex(price, idx)));
  });
}

function parseSupportListClauses(text, instrument, levelMap, parse_errors) {
  const supportPrefixRe = /\bsupports?\s*(?:are|=|:)\s*([^.\n]+)/gi;
  let prefixed;
  while ((prefixed = supportPrefixRe.exec(text)) !== null) {
    const snippet = getSnippet(text, prefixed.index, prefixed.index + prefixed[0].length, 50);
    addClauseLevels(levelMap, parse_errors, instrument, prefixed[1], snippet, () => ({
      significance: 'key',
      direction: 'support',
      intent: null,
    }));
  ...
  const supportSuffixRe = /((?:\d{4,5}(?:\.\d+)?(?:\s*\([^)]*\))?\s*(?:,|and|&|\s)+){1,}\d{4,5}(?:\.\d+)?(?:\s*\([^)]*\))?)\s*(?:=|are|were)\s*supports?\b/gi;
  let suffixed;
  while ((suffixed = supportSuffixRe.exec(text)) !== null) {
    const snippet = getSnippet(text, suffixed.index, suffixed.index + suffixed[0].length, 50);
    addClauseLevels(levelMap, parse_errors, instrument, suffixed[1], snippet, () => ({
      significance: 'key',
      direction: 'support',
      intent: null,
    }));
...
function parseTargetListClauses(text, instrument, levelMap, parse_errors) {
  const targetClauses = [
    /\bsets?\s+up\s+([^.\n]+)/gi,
    /\btargets?\s+(?:given\s+at\s+\d{1,2}\s*am\s+)?(?:were|are|was|set)\s*([^.\n]+)/gi,
    /\bbonus\s+set\s+(?:were|was|is|are)\s*([^.\n]+)/gi,
  ];

  for (const re of targetClauses) {
    let match;
    while ((match = re.exec(text)) !== null) {
      const snippet = getSnippet(text, match.index, match.index + match[0].length, 50);
      if (/\ball\s+hit\b/i.test(match[1]) || /\ball\s+hit\b/i.test(snippet)) {
        parse_errors.push(`all-hit target list recorded as context only from "${snippet}"`);
        continue;
      }
      const kind = /\bbonus\b/i.test(match[0]) ? 'bonus' : 'core';
      addClauseTargets(levelMap, parse_errors, instrument, text, match[1], snippet, kind);
    }
  }
}
...
function parseManciniText(rawText, livePrices = null) {
  ...
  parseSupportListClauses(analysisText, instrument, levelMap, parse_errors);
  parseTargetListClauses(analysisText, instrument, levelMap, parse_errors);
  ...
  while ((firstTarget = firstTargetRe.exec(analysisText)) !== null) {
    ...
    if (/\ball\s+hit\b/i.test(snippet)) continue;
    if (!validatePrice(instrument, price, parse_errors, snippet)) continue;
    pushOrUpgrade(levelMap, buildLevel(price, snippet, {
