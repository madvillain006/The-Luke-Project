...
// ── Aspirational language detector ───────────────────────────
const ASPIRATIONAL_RE = /\b(?:i['']?d\s+(?:still\s+)?(?:love|like)|would\s+(?:love|like)|hoping|looking\s+to|love\s+an?\s+opportunity|ideally)\b/i;
...
function parseXimes(username, rawText) {
  ...
  const isAspirational = ASPIRATIONAL_RE.test(text);

  // Pattern F: bare direction word with session strike
  if (!strike && !isAspirational && /\bputs?\b/i.test(text)) direction = 'SHORT';
  if (!strike && !isAspirational && /\bcalls?\b/i.test(text)) direction = 'LONG';

  // Pattern G: ticker + bare number (no direction qualifier) "SPX 7105"
  if (!strike && !isAspirational && ticker) {
