...
function parseXimes(username, rawText) {
  ...
  // Username gating — must be a known analyst
  const analyst = username ? ANALYST_MAP[username.toLowerCase()] : null;
  if (!analyst) return null;
  const isRelay  = analyst.role === 'relay';
  const analystId = analyst.source;
