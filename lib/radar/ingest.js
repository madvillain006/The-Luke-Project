...
function summarizeRadarItem(row, review = null) {
  ...
  return {
    ...
    review_only: row.review_only === true,
    trading_authority: row.trading_authority || 'none',
...
function normalizeScope(value, sourceType = 'manual_paste') {
  const raw = String(value || '').trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '_').replace(/^_+|_+$/g, '');
  if (raw) return raw.slice(0, 60);
  if (sourceType === 'reference_idea') return 'reference_review';
  if (sourceType === 'katbot_paste') return 'katbot_confluence';
  return 'general';
}

function normalizeStatus(value, sourceType = 'manual_paste') {
  const raw = String(value || '').trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '_').replace(/^_+|_+$/g, '');
  if (raw) return raw.slice(0, 60);
  if (sourceType === 'reference_idea' || sourceType === 'katbot_paste' || sourceType === 'pine_trading_note') {
    return 'review_only';
  }
  return 'captured';
}

function defaultRecallReason(sourceType, themes = []) {
  if (sourceType === 'reference_idea') return 'reference_idea_review_lane';
  if (sourceType === 'katbot_paste') return 'katbot_confluence_evidence';
  if (themes.includes('contradiction')) return 'contradiction_or_risk_signal';
  if (themes.includes('reminder')) return 'reminder_or_follow_up';
  return 'source_freshness_or_keyword_match';
}
...
function recordRadarIngest(input = {}, options = {}) {
  ...
  const scope = normalizeScope(input.scope || input.lane || input.review_scope || input.reviewScope, sourceType);
  const status = normalizeStatus(input.status || input.review_status || input.reviewStatus, sourceType);
  const recallReason = compactText(input.recall_reason || input.recallReason || defaultRecallReason(sourceType, themes), 140);
  const reviewOnly = sourceType === 'reference_idea' || sourceType === 'katbot_paste' || sourceType === 'pine_trading_note' || input.review_only === true || input.reviewOnly === true;
  const item = {
