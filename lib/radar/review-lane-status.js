...
function buildQaSection(qaPackets = []) {
  if (!qaPackets.length) {
    return {
      available: true,
  ...
  const latestPacket = latestQaPacket(qaPackets);
  return {
    available: true,
    persisted_state_available: false,
    summary_line: `${counts.valid}/${counts.total} QA packet(s) valid; success claims remain evidence-bound.`,
    latest_packet_at: asString(latestPacket?.timestamp || latestPacket?.deterministic_equivalent) || null,
...
function buildReviewLaneStatusReport(input = {}, options = {}) {
  ...
  const qaPackets = toArray(input.qa_packets || input.qaPackets);
  ...
  const persistedStateAvailable = evidence.persisted_state_available;
  ...
  return {
    ...
    summary_line: [
      ...
      persistedStateAvailable
        ? 'Live read-only Radar evidence is available for review.'
