...
function formatDecisionFreshnessLine(freshness) {
  ...
  const parts = [
    formatLoadedFlag(Boolean(freshness.saty?.loaded), 'Saty'),
...
function formatDecisionTiming(decision, currentPrice) {
  if (!Number.isFinite(currentPrice)) return 'WAIT - market price unavailable';
...
function renderEntriesDecision({ instrument, currentPrice, decision, tradeState, katEntryLine = null }) {
  ...
  const currentLine = Number.isFinite(currentPrice)
    ...
    : `Current: ${instrument} market price unavailable`;
  const timing = formatDecisionTiming(decision, currentPrice);
