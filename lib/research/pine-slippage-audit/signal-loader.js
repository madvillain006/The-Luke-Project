...
function loadPineHardmodeConfig(rootDir = ROOT) {
  ...
  const hardmodePath = path.join(rootDir, 'tradingview', 'history', 'level-reclaim', 'luke-level-reclaim-watch-hardmode.strategy.pine');
  ...
  return {
    ...
    files: {
      visual_indicator: path.join(rootDir, 'tradingview', 'history', 'level-reclaim', 'luke-level-reclaim-watch.pine'),
