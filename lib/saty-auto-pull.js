...
function scaleBars(bars, factor) {
  return (bars || []).map(bar => ({
    ...bar,
    open: round2(bar.open * factor),
    high: round2(bar.high * factor),
    low: round2(bar.low * factor),
    close: round2(bar.close * factor),
...
async function captureReferenceSnapshot(options = {}) {
  ...
  if (!key) return { ok: false, error: 'MASSIVE_API_KEY not set' };

  const livePrice = await getLivePrice().catch(() => null);
  let dailyBars;
  let dailySource = 'polygon:I:SPX';
  ...
  try {
    ...
    const spyBars = await fetchDailyBarsFromPolygon('SPY', key, options.daysBack || DEFAULT_HISTORY_DAYS);
    if (!Array.isArray(spyBars) || spyBars.length < 35) {
      return { ok: false, error: `SPX fetch failed (${spxErr.message}); SPY fallback unavailable` };
    }
    dailyBars = scaleBars(spyBars, 10);
    dailySource = 'polygon:SPYx10-fallback';
  }
  ...
  return {
    ...
    snapshot: {
      ...
      source: livePrice?.source || 'massive',
