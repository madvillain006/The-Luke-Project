...
const { isFuturesMarketOpen, isMarketOpen } = require('./market-hours');
...
const DEFAULT_US500_YAHOO_SYMBOLS = ['US500'];
...
function shouldPreferUs500Reference(options = {}) {
  if (options.preferUs500 === false) return false;
  if (options.preferUs500 === true) return true;
  const now = options.now || new Date();
  const futures = isFuturesMarketOpen(now);
  const cash = isMarketOpen(now);
  return futures.open === true && cash.open !== true;
}

async function fetchFirstYahooBars(symbols, options = {}) {
  const errors = [];
  for (const symbol of symbols) {
    try {
      const bars = await fetchDailyBarsFromYahoo(symbol, {
        range: options.yahooRange || '6mo',
        timeoutMs: options.timeoutMs,
      });
      if (!Array.isArray(bars) || bars.length < 35) throw new Error('not enough daily bars');
      return { ok: true, bars, source: `yahoo:${symbol}`, errors };
    } catch (err) {
      errors.push(`yahoo:${symbol} ${err.message}`);
    }
  }
  return { ok: false, bars: null, source: null, errors };
}
...
async function captureReferenceSnapshot(options = {}) {
  ...
  const errors = [];
  const preferUs500 = shouldPreferUs500Reference(options);
  ...
  if ((!Array.isArray(dailyBars) || dailyBars.length < 35) && preferUs500) {
    const symbols = parseSymbolList(options.us500YahooSymbols || process.env.SATY_US500_YAHOO_SYMBOLS, DEFAULT_US500_YAHOO_SYMBOLS);
    const result = await fetchFirstYahooBars(symbols, options);
    if (result.ok) {
      dailyBars = result.bars;
      dailySource = result.source;
    } else {
      errors.push(...result.errors);
    }
    referenceAttempts.push(...result.errors);
  }
  ...
  return {
    ...
    snapshot: {
      ...
      reference_preference: preferUs500 ? 'us500_when_futures_open' : 'spx_cash',
      reference_attempt_errors: referenceAttempts,
...
async function runSatyAutoPull(options = {}) {
  ...
  const baseState = {
    ...
    reference_snapshot: {
      ...
      reference_preference: captured.snapshot.reference_preference,
      reference_attempt_errors: captured.snapshot.reference_attempt_errors,
