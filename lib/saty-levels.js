...
function parseSatyText(text) {
  ...
  // Strip thousands-separator commas (e.g. "5,865.25" → "5865.25") before matching
  const normalized = text.replace(/(\d),(\d{3})(?!\d)/g, '$1$2');

  const nums = [...normalized.matchAll(/(?<![.\d])(\d{3,6}(?:\.\d+)?)(?![.\d])/g)]
...
function buildStatusSummary(satyData) {
  if (!satyData) return 'Saty is generated automatically from previous close levels. /saty is only a manual override/debug command.';
  ...
  lines.push(`\nManual override only: paste 13 Saty levels (space or newline separated)`);
  return lines.join('\n');
}

async function appendSatyToMemory(satyData) {
  if (!satyData || satyData.valid !== true) {
    throw new Error('appendSatyToMemory: valid saty data required');
  }

  const timestamp = satyData.updated || new Date().toISOString();
  const results = [];

  for (const { field, significance } of SATY_MEMORY_FIELDS) {
    ...
    results.push({ field, price, ...result });
  }
