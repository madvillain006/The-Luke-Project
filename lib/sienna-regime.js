...
const BOBBY_CONTEXT_FILE = path.join(JARVIS_ROOT, 'bobby-context.jsonl');
const HISTORY_FILE = path.join(JARVIS_ROOT, 'discord-history.jsonl');
...
function getSiennaRegime() {
  ...
  if (todayXimes.length === 0) {
    return { regime: 'NEUTRAL', selectivity: 'HIGH', max_trades_today: 2, confidence_boost: 0, reason: 'No signals yet today' };
  }
