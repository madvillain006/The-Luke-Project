...
async function handleHeatmap(message, res, ctx) {
  if (message.trim() === '/heatmap' && !res._heatmapImage) {
    return res.json({ reply: "HEATMAP Paste KatBot/SPX heatmap text then run:\n/heatmap [paste text here]\n\nOr paste the image directly into chat; vision will parse it when vision routing is enabled." });
  }
  ...
  if (merged) {
    const storedBobby = {
      ...
      channel: merged.channel || 'bobby-spx-coms',
    };
    obj.bobby = [...(obj.bobby || []), storedBobby];
    ctx.saveLevels(obj);
    fs.appendFileSync(BOBBY_CONTEXT_JSONL, JSON.stringify(storedBobby) + '\n');
    await appendBobbyToMemory(storedBobby);
    const nodeCount = (storedBobby.king_nodes || []).length + (storedBobby.support || []).length + (storedBobby.resistance || []).length;
