...
async function handleSlashCommand(message, res) {
  ...
  if (message.startsWith("/alert ")) {
    ...
    let signal = parseXimes(text);
    let detectedUsername = null;

    const uFromText = text.match(/\[\d{1,2}:\d{2}\s*[AP]M\]\s+(\S+)/i);
    if (uFromText) detectedUsername = uFromText[1].toLowerCase();

    if (!signal) return res.json({ reply: "❌ SKIP — could not parse signal" });

    if (signal.type === 'MANAGEMENT') {
      if (signal.action === 'TRIM') {
        return res.json({ reply:
          '⚡ XIMES TRIM — Take partial profits now.\n' +
          (signal.gainPct ? 'He called ' + signal.gainPct +
            '% gain.\n' : '') +
