...
async function handleSlashCommand(message, res) {
  ...
  if (message.trim() === "/trading-mode") {
    ...
    return res.json({ reply: '🖥️ Layout launching — ximes top-left, Bobby bottom-left, Tradovate top-right, Jarvis bottom-right. Takes ~20s. Check top bar for errors.' });
  }
  ...
  if (message.startsWith('/saty ')) {
    ...
    if (!satyParsed.valid) {
      return res.json({ reply: `❌ ${satyParsed.error || 'parse failed'}\n\nFormat: /saty up2=5920 up1=5895 mid=5870 down1=5845 down2=5820\nOr bare numbers top-to-bottom: /saty 5920 5895 5870 5845 5820\nOptional: add ribbon=bullish or ribbon=bearish` });
    }
    saveSatyLevels(satyParsed);
    const satyLevelLines = Object.entries(satyParsed)
      .filter(([k]) => ['up2','up1','mid','down1','down2'].includes(k))
      .map(([k, v]) => {
        const label = k === 'up2' ? 'ATR+2' : k === 'up1' ? 'ATR+1' : k === 'mid' ? 'Mid  ' : k === 'down1' ? 'ATR-1' : 'ATR-2';
        return `  ${label}: ${v}`;
      }).join('\n');
    const satyRibbonLine = satyParsed.ribbon ? `\nRibbon: ${satyParsed.ribbon}` : '';
    const satyAtrLine    = satyParsed.atr    ? `\nATR value: ${satyParsed.atr} pts` : '';
    return res.json({ reply: `📐 Saty ATR levels saved:\n${satyLevelLines}${satyRibbonLine}${satyAtrLine}\n\nWill appear in /alert as entry refinement when a signal is near a level.` });
  }
