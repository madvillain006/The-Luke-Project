...
async function handleSlashCommand(message, res) {
  ...
  function checkSessionReadiness() {
    ...
    try {
      ...
      // 4. EMOTIONAL_CLEAR — already handled by existing hard block below
    } catch (e) {
  ...
  if (message.startsWith("/alert ")) {
    ...
    return res.json({ reply: readinessPrefix + runnerWarning + `⚠️ WEAK — ${hit.confidence} confluence at ${hit.level} (${instrLabel}) (score: ${hit.score})${timeSuffix}${regimeLine}${edgeSuffix}${softSuffix}` });
  }
