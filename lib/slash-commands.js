...
function handleSlashCommand(message, res) {
  ...
  if (message.startsWith("/alert ")) {
    ...
    if (hit.confidence === "HIGH") {
      ...
      let stopVal, targetVal, stopSource, targetSource;

      if (direction === "LONG") {
        const above = zones.filter(z => z.level > strike + tol && (z.confidence === "HIGH" || z.confidence === "MEDIUM"))
          .sort((a, b) => a.level - b.level);
        const below = zones.filter(z => z.level < strike - tol).sort((a, b) => b.level - a.level);
        if (above.length > 0) { targetVal = above[0].level; targetSource = `${above[0].confidence} zone`; }
        if (below.length > 0) { stopVal = below[0].level; stopSource = "zone below"; }
      } else if (direction === "SHORT") {
        const below = zones.filter(z => z.level < strike - tol && (z.confidence === "HIGH" || z.confidence === "MEDIUM"))
          .sort((a, b) => b.level - a.level);
        const above = zones.filter(z => z.level > strike + tol).sort((a, b) => a.level - b.level);
        if (below.length > 0) { targetVal = below[0].level; targetSource = `${below[0].confidence} zone`; }
        if (above.length > 0) { stopVal = above[0].level; stopSource = "zone above"; }
      }

      if (!stopVal) {
        const stopDist = instrument === 'ES_NQ' ? 10 : 1.5;
        stopVal = direction === "SHORT" ? strike + stopDist : strike - stopDist;
        stopSource = "fixed";
      }
      if (!targetVal) {
        const targetDist = instrument === 'ES_NQ' ? 20 : 3.0;
        targetVal = direction === "SHORT" ? strike - targetDist : strike + targetDist;
        targetSource = "fixed";
      }

      const risk = Math.abs(strike - stopVal);
      const reward = Math.abs(targetVal - strike);
      const rr = risk > 0 ? (reward / risk).toFixed(1) : "N/A";

      const ticker = signal.ticker || instrLabel;
      saveLastSignal(ticker, "SETUP");
      log("alert", { event: "stop+target from confluence zones", stopSource, targetSource });
      return res.json({ reply: `✅ SETUP — ${ticker} ${strike}${timeSuffix}\nZone: ${hit.level} ${hit.instrument || instrLabel} ${hit.bias} (score: ${hit.score})\nStop: ${typeof stopVal === "number" ? stopVal.toFixed(2) : stopVal} (${stopSource || "zone"}) | Target: ${typeof targetVal === "number" ? targetVal.toFixed(2) : targetVal} (${targetSource || "zone"})\nR:R ${rr}:1${regimeLine}${edgeSuffix}${softSuffix}` });
    }
