...
const { getKatbotRegime } = require("./katbot-regime");
...
function cleanReplyText(text) {
  ...
  if (out.includes('Paste a Ximes signal then run:')) {
    return "ALERT LEGACY Manual /alert is retired from the main Luke Watch Pine workflow.\nUse TradingView/Pine alerts plus /heatmap, /verdict ES, and /entries ES.";
  }
...
function getMorningPrepLine(apexState, workflowStatus) {
  const now = new Date();
...
function checkSessionReadiness() {
  ...
  try {
    ...
    if (!workflowStatus.bobbyLoaded) {
      warnings.push("Warning: Katbot/SPX heatmap not loaded yet. Run /heatmap before trading.");
      ...
      if (bobby.length > 0) {
        ...
        if (age > 8 * 3600 * 1000) warnings.push("Warning: Katbot/SPX heatmap is stale (>8h old). Reload /heatmap before trading.");
      }
    }
...
async function handleSlashCommand(message, res) {
  ...
  if (message.trim() === "/ready") {
    ...
    checks.push('OK Saty generated from previous close levels');

    const workflowStatus = getPhase2WorkflowLoadStatus();
    checks.push(workflowStatus.bobbyLoaded
      ? `OK Katbot/SPX heatmap loaded (${workflowStatus.bobbyCount} mentions)`
      : 'X Katbot/SPX heatmap NOT loaded - paste heatmap and run /heatmap');
  ...
  if (message.trim() === "/alert") {
    return res.json({ reply: "ALERT LEGACY Manual /alert is retired from the main Luke Watch Pine workflow.\nUse TradingView/Pine alerts plus /heatmap, /verdict ES, and /entries ES." });
  }
...
${readiness.warnings.join('\n')}\n\n`
  ...
  if (message.startsWith("/luke ") || message.trim() === "/luke") {
    return res.json({
      reply: [
        ...
        "Use the same front chat for general conversation, trading readiness, alerts, balances, and level-state work.",
        "Luke routes the lane internally and keeps shared context bins."
      ].join("\n")
    });
  }
  ...
  if (message.trim() === "/layout") {
    ...
    return res.json({
      reply: [
        " Layout starting...",
        "1. Positioning Luke ",
        "2. Opening Pine/TradingView watch",
        "3. Opening Katbot/SPX heatmap context",
  ...
  if (message.trim() === "/trading-mode") {
    ...
    return res.json({ reply: 'Layout launching: Pine/TradingView top-left, Katbot heatmap top-right, Tradovate bottom-left, Luke bottom-right. Takes ~20s. Check top bar for errors.' });
  }
  ...
  //
  // C7: fails open when Level Memory is empty or live price is UNKNOWN/not trusted.
