...
async function handleSlashCommand(message, res) {
  if (message.trim() === "/levels") {
    return res.json({ reply: "📋 Legacy path. Use /dubz for the current RichyDubz workflow.\n\nExample:\n/dubz [paste RichyDubz morning text]" });
  }
  ...
  if (message.trim() === "/confluence") {
    ...
    if ((obj.richyd || []).length === 0 && (obj.bobby || []).length === 0) {
      return res.json({ reply: "⚠️ No levels loaded today. Paste /dubz and /heatmap first." });
    }
  ...
  function checkSessionReadiness() {
    ...
    try {
      ...
      if (!workflowStatus.dubzLoaded) {
        warnings.push("⚠️ No levels loaded — paste RichyDubz and run /dubz");
      }
  ...
  if (message.trim() === "/ready") {
    ...
    if (workflowStatus.dubzLoaded) {
      checks.push(`✅ RichyDubz levels loaded (${workflowStatus.dubzCount} levels)`);
    } else {
      checks.push("❌ RichyDubz levels NOT loaded — paste /dubz");
    }

    // 3. Bobby heatmap loaded
    if (workflowStatus.bobbyLoaded) {
      checks.push(`✅ Bobby heatmap loaded (${workflowStatus.bobbyCount} mentions)`);
    } else {
  ...
  if (message.startsWith('/mancini ')) {
    ...
    if (parsed.levels.length === 0 && parsed.chop_zones.length === 0) {
      return res.json({ reply: `âŒ Mancini parse failed.\n${parsed.parse_errors.join('\n')}` });
    }
    ...
    const lines = [`ðŸ“˜ Mancini levels saved (${parsed.instrument} ${parsed.date})`];
  ...
  if (message.trim() === '/verdict' || message.startsWith('/verdict ')) {
    ...
    // Get live prices for staleness grounding — fail-open if unavailable (C7)
    let livePrice = null;
    try {
      livePrice = await getLivePrice();
    } catch { /* no-op — staleness flags will be absent */ }
