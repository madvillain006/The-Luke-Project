...
// â”€â”€ ALERT DEDUP (BLOCKER 2) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const recentAlerts = new Map(); // key â†’ timestamp
...
function checkAlertDedup(signal) {
  ...
  if (last && now - last < ALERT_DEDUP_MS) {
    ...
    return `ðŸ” Duplicate alert (fired ${ago}s ago). Ignoring.`;
  }
  recentAlerts.set(key, now);
  return null;
}

// â”€â”€ APEX STATE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ ACTIVE TRADE STATE (BLOCKER 3) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ VISION RATE LIMIT (BLOCKER 1) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function handleSlashCommand(message, res) {
  if (message.trim() === "/levels") {
    return res.json({ reply: "ðŸ“‹ Legacy path. Use /dubz for the current RichyDubz workflow.\n\nExample:\n/dubz [paste RichyDubz morning text]" });
  }
  ...
  if (message.trim() === "/heatmap" && !res._heatmapImage) {
    return res.json({ reply: "ðŸŒ¡ï¸ Paste Bobby's heatmap text then run:\n/heatmap [paste text here]\n\nOr paste the image directly into chat â€” vision will parse it automatically." });
  }
  ...
  if (message.trim() === "/heatmap" || message.startsWith("/heatmap ")) {
    ...
    if (res._heatmapImage) {
      if (now - lastVisionCallMs < VISION_RATE_LIMIT_MS) {
        ...
        visionNote = ` (vision rate-limited â€” retry in ${waitSecs}s)`;
      } else {
        ...
        try {
          ...
          if (visionResult && visionResult.parse_status === 'failed') {
            visionFailed = true;
            visionNote = `\nâŒ Vision error: ${visionResult.error}`;
            ...
            visionFailed = true;
            visionNote = `\nâŒ Vision returned no data`;
          }
        } catch (err) {
          visionFailed = true;
          visionNote = `\nâŒ Vision error: ${err.message}`;
        }
      }
    }
    ...
    if (merged) {
      ...
      const heatmapPrefix = mixedSuccess ? 'âš ï¸ Partial parse â€” text levels captured, image vision failed\n' : '';
    ...
    return res.json({ reply: `${visionFailed && !textBobby ? 'âŒ ' : ''}No prices found in heatmap text${visionNote}.` });
  }
  ...
  if (message.trim() === "/confluence") {
    ...
    if ((obj.richyd || []).length === 0 && (obj.bobby || []).length === 0) {
      return res.json({ reply: "âš ï¸ No levels loaded today. Paste /dubz and /heatmap first." });
    }
    ...
    if (displayZones.length === 0) {
      return res.json({ reply: `ðŸ“Š TODAY'S CONFLUENCE ZONES\n${sep}\nNo HIGH or MEDIUM zones yet.\n${sep}\nLoaded: ${richydCount} RichyDubz levels Â· ${bobbyCount + extraBobby.length} Bobby nodes` });
    }
    const zoneLines = displayZones.map(z => {
      const icon = z.confidence === "HIGH" ? "ðŸ”´" : "ðŸŸ¡";
  ...
  if (message.trim() === "/balance") {
    ...
    if (apexNow) {
      return res.json({ reply: `ðŸ’° Apex balance: ${apexNow.balance?.toLocaleString()} | Floor: ${apexNow.trail_floor?.toLocaleString()} | Headroom: ${(apexNow.balance - apexNow.trail_floor)?.toLocaleString()}\nLast updated: ${new Date(apexNow.updated).toLocaleString()}\n\nTo update: /balance 51200` });
    }
    return res.json({ reply: "ðŸ’° No balance set yet. Run:\n/balance [your current Apex balance]\nExample: /balance 50717" });
  }

  if (message.startsWith("/balance ")) {
    const raw = message.slice(9).trim().replace(/[$,]/g, "");
    const bal = parseFloat(raw);
    if (!bal || isNaN(bal)) return res.json({ reply: "âŒ Format: /balance 51200" });
  ...
  function checkSessionReadiness() {
    ...
    try {
      ...
      if (!apexState || !apexState.updated) {
        warnings.push("âš ï¸ /balance not run today â€” floor check may be stale");
      } else {
        const todayET = new Date().toLocaleDateString('en-CA', { timeZone: 'America/New_York' });
        const balanceET = new Date(apexState.updated).toLocaleDateString('en-CA', { timeZone: 'America/New_York' });
        if (balanceET !== todayET) warnings.push("âš ï¸ /balance not run today â€” floor check may be stale");
      }

      // 2. LEVELS_LOADED
      const workflowStatus = getPhase2WorkflowLoadStatus();
      if (!workflowStatus.dubzLoaded) {
        warnings.push("âš ï¸ No levels loaded â€” paste RichyDubz and run /dubz");
      }

      // 3. BOBBY_LOADED
      if (!workflowStatus.bobbyLoaded) {
        warnings.push("âš ï¸ No Bobby context â€” paste heatmap and run /heatmap");
        ...
        if (bobby.length > 0) {
          ...
          if (age > 8 * 3600 * 1000) warnings.push("âš ï¸ Bobby context is stale (>8h) â€” repaste /heatmap");
        }
      }
      ...
      if (apexFloorState && typeof apexFloorState.balance === 'number' && typeof apexFloorState.trail_floor === 'number') {
        if (apexFloorState.balance - apexFloorState.trail_floor <= 500) {
          warnings.push("âš ï¸ Apex floor headroom < $500 â€” size down or skip");
        }
      }

      // 5. EMOTIONAL_CLEAR â€” already handled by existing hard block below
    } catch (e) {
  ...
  if (message.trim() === "/ready") {
    ...
    if (readyApex && readyApex.updated) {
      ...
      checks.push(balET === todayET ? "âœ… Balance set today" : "âŒ Balance NOT set today â€” run /balance");
    } else {
      checks.push("âŒ Balance NOT set today â€” run /balance");
    }

    const workflowStatus = getPhase2WorkflowLoadStatus();

    // 2. RichyDubz/Dubz levels loaded
    if (workflowStatus.dubzLoaded) {
      checks.push(`âœ… RichyDubz levels loaded (${workflowStatus.dubzCount} levels)`);
    } else {
      checks.push("âŒ RichyDubz levels NOT loaded â€” paste /dubz");
    }

    // 3. Bobby heatmap loaded
    if (workflowStatus.bobbyLoaded) {
      checks.push(`âœ… Bobby heatmap loaded (${workflowStatus.bobbyCount} mentions)`);
    } else {
      checks.push("âŒ Bobby heatmap NOT loaded â€” paste heatmap and run /heatmap");
    }
    ...
    if (readyApex && typeof readyApex.balance === 'number' && typeof readyApex.trail_floor === 'number') {
      ...
      if (headroom > 500) {
        checks.push(`âœ… Apex floor safe ($${headroom.toLocaleString()} headroom)`);
      } else {
        checks.push(`âŒ Apex floor headroom < $500 ($${headroom.toLocaleString()}) â€” size down or skip`);
      }
    } else {
      checks.push("âŒ Apex floor unknown â€” run /balance first");
    }

    const allPass = checks.every(c => c.startsWith("âœ…"));
    const verdict = allPass ? "âœ… READY TO TRADE" : "âŒ NOT READY â€” fix above before trading";
    return res.json({ reply: `ðŸš¦ SESSION READINESS\n${sep}\n${checks.join("\n")}\n${sep}\n${verdict}` });
  }

  if (message.trim() === "/alert") {
    return res.json({ reply: "ðŸ“¡ Paste a Ximes signal then run:\n/alert [paste signal here]\n\nExample:\n/alert [2:34 PM] ximestrades ES LONG 5880" });
  }
  ...
  if (message.startsWith("/alert ")) {
    ...
    if (isWeekend()) {
      ...
      return res.json({ reply: "âŒ Market closed. Weekend." });
    }
    if (!isMarketOpen().open) {
      ...
      return res.json({ reply: "â° Market closed. Opens 9:30 AM ET. Use this time to prep levels." });
    }
    ...
    if (!_tradingTime.good) {
      if (_tradingTime.window === "lunch") {
        ...
        return res.json({ reply: "âš ï¸ Lunch chop window. High risk. Skip unless A+ setup only." });
      }
      if (_tradingTime.window === "last10") {
        ...
        return res.json({ reply: "âš ï¸ Last 10 mins. Ximes says no responsibility after 3:49 PM." });
      }
    }
    ...
    if (!signal) {
      ...
      return res.json({ reply: "âŒ SKIP â€” could not parse signal" });
    }
    ...
    if (signal.signal_type === 'MANAGEMENT') {
      if (signal.action === 'TRIM') {
        ...
        if (trimActive && trimActive.status === 'open') {
          ...
          runnerLine = '\nðŸƒ Runner set â€” ' + (100 - trimPct) + '% remaining.' +
                       '\nâ†’ Stop: move to breakeven (' + trimActive.strike + ')' +
                       '\nâ†’ Let Ximes call the exit. Do not touch it.';
        }
        ...
        return res.json({ reply:
          'âœ‚ï¸ TRIM â€” Ximes taking ' + trimPct + '% off.\n' +
          'â†’ Reduce position by ' + trimPct + '% NOW in Tradovate.' +
          runnerLine
        });
      }
      if (signal.action === 'RUNNER') {
        ...
        return res.json({ reply:
          'ðŸƒ XIMES RUNNER â€” Hold partial position.\n' +
      ...
      if (signal.action === 'CLOSE') {
        ...
        return res.json({ reply:
          'ðŸšª XIMES EXIT â€” Close position now.' });
      }
      if (signal.action === 'ADD') {
        ...
        return res.json({ reply:
          'âž• XIMES ADDING â€” He is sizing in further.\n' +
      ...
      if (signal.action === 'STOP_UPDATE') {
        ...
        return res.json({ reply:
          'âº STOP UPDATE â€” Move your stop to ' + price + '.\n' +
          'â†’ Adjust in Tradovate now. Do not wait.'
        });
      }
      if (signal.action === 'TARGET_UPDATE') {
        ...
        return res.json({ reply:
          'ðŸŽ¯ TARGET UPDATE â€” ' + ticker + 'new target: ' + price + '.\n' +
          'â†’ Adjust your OCO or mental target now.'
        });
      }
    }
    ...
    // BLOCKER 3 â€” active trade guard
    let runnerWarning = '';
    const activeTrade = loadActiveTrade();
    if (activeTrade) {
      if (activeTrade.runner) {
        runnerWarning = 'âš ï¸ Runner active on ' + (activeTrade.ticker || '?') +
                        ' (' + (activeTrade.runner_pct || '?') + '% remaining). ' +
                        'New signal is independent â€” manage runner separately.\n\n';
      } else {
        ...
        return res.json({ reply: `âš ï¸ Active trade detected (${activeTrade.direction} ${activeTrade.ticker} opened ${since}m ago). Close current trade with /trade before new signals.` });
      }
    }
    ...
    if (!strike) {
      ...
      return res.json({ reply: "âŒ SKIP â€” no strike found in signal" });
    }
    ...
    if (_isMorning && (instrLabel === "SPY_QQQ" || instrLabel === "SPX")) {
      edgeNote = "ðŸ“‹ Edge match: Ximes morning window (9:30â€“11:00 AM) â€” preferred setup";
    } else if (_isMorning) {
      edgeNote = "ðŸ“‹ Edge match: Morning window â€” Ximes preferred SPY/SPX 0-1DTE play";
    } else if (_isAfternoon) {
      edgeNote = "ðŸ“‹ Edge match: Afternoon window (2â€“3:50 PM) â€” higher risk, size down âš ï¸";
    }
    ...
    if (!hit) {
      ...
      return res.json({ reply: runnerWarning + `âŒ SKIP â€” no confluence at ${strike} (${instrLabel}) today${timeSuffix}${softSuffix}` });
    }
    ...
    const regimeLine = `\nðŸŒ¡ï¸ Regime: ${regime.regime} â€” ${regime.reason}`;
    ...
    if (todayTradeCount >= regime.max_trades_today) {
      ...
      return res.json({ reply: `ðŸš« BLOCKED â€” max ${regime.max_trades_today} trades today reached [${regime.regime}]${regimeLine}${timeSuffix}${softSuffix}` });
    }
    ...
    if (hit.confidence === "HIGH") {
      ...
      if (bracket.error) {
        ...
        return res.json({ reply: `âŒ SKIP â€” bracket error: ${bracket.error}` });
      }
      ...
      if (apexState && apexState.balance && apexState.trail_floor) {
        ...
        if (wouldBreach) {
          ...
          return res.json({ reply: `â›” APEX FLOOR â€” This trade risks breaching your EOD trail floor ($${apexState.trail_floor.toLocaleString()}). Max risk on this trade: $${maxRisk.toLocaleString()}. Reduce size or skip.` });
        }
      }
      ...
      // BLOCKER 3 â€” mark trade as active on SETUP
      if (verdict === "SETUP") {
        writeActiveTrade({ ...signal, ticker, direction }, bracket, sizingNote ? { sizing_note: sizingNote } : {});
        // Fire popup via WS â€” same staged_trade event the autonomous path uses
        if (typeof global.broadcast === 'function') {
          ...
          global.broadcast({
            type:         'staged_trade',
            signal:       { ticker, direction, entry: bracket.entry, stop: bracket.stop, target: bracket.target, confluence_score: hit.score, confluence_confidence: hit.confidence, reason: `${hit.confidence} confluence â€” manual alert` },
            trade:        tradePayload,
            risk_dollars: bracket.risk_dollars,
            rr:           bracket.rr_ratio,
            seconds_left: 60,
      ...
      const katContextLine = katSummary
        ? '\nðŸŽ¯ Kat context (' + katInstrument + '): ' +
          katSummary.count + ' signals in last 30m â€” ' + katSummary.dominant_bias +
      ...
      const replyText = [
        `${verdictEmoji} ${verdict} â€” ${ticker} ${direction}${timeSuffix}`,
        `Entry:  ${bracket.entry}`,
        `Stop:   ${stopLine}`,
        `Target: ${targetLine}`,
    ...
    const _katWeak = katSummary
      ? '\nðŸŽ¯ Kat context (' + katInstrument + '): ' +
        ...
        (katSummary.with_image ? ' (' + katSummary.with_image + ' charts)' : '')
      : '';
    return res.json({ reply: readinessPrefix + runnerWarning + `âš ï¸ WEAK â€” ${hit.confidence} confluence at ${hit.level} (${instrLabel}) (score: ${hit.score})${timeSuffix}${regimeLine}${edgeSuffix}${softSuffix}` + _katWeak });
  }
  ...
  if (message.startsWith('/runner')) {
    ...
    if (!runnerTrade || runnerTrade.status !== 'open') {
      return res.json({ reply: 'âŒ No active trade to set runner on.' });
    }
    ...
    return res.json({ reply:
      'ðŸƒ Runner active.\n' +
      'â†’ ' + pct + '% trimmed. ' + (100 - pct) + '% running.\n' +
      'â†’ Move stop to breakeven (' + runnerTrade.strike + ') in Tradovate NOW.\n' +
      'â†’ Next Ximes management signal will guide the rest.'
    });
  }
  ...
  if (message.startsWith("/trade ")) {
    ...
    if (!isRunnerClose) {
      ...
      if (_runnerCheck && _runnerCheck.runner === true) {
        ...
        return res.json({ reply:
          `âš ï¸ RUNNER ACTIVE â€” ${_rt} ${_rp}% still running.\n` +
          `Ximes hasn't called the exit yet.\n` +
          `â†’ Close full position? Add RUNNER to confirm: /trade ${direction} ${ticker} ${rawEntry} ${rawExit} ${result} RUNNER\n` +
          `â†’ Or set runner stop in Tradovate and wait for Ximes call.`
        });
      }
    }
    appendJsonl(TRADES_JSONL, tradeEntry);
    // F1 fix â€” update state.daily_pnl so Apex consistency cap sees manual trades
    try {
    ...
    clearActiveTrade(); // BLOCKER 3 â€” trade closed, clear active-trade guard
    const sign = pnl >= 0 ? "+" : "";
    const runnerSuffix = isRunnerClose ? ' Runner closed. Full position closed.' : '';
    return res.json({ reply: `âœ… Trade logged. ${direction} ${ticker} ${entry}â†’${exit} ${result} (${sign}${pnl} pts)${runnerSuffix}` });
  }
  ...
  if (message.trim() === "/review") {
    ...
    const sign = netPts >= 0 ? "+" : "";
    const header = `ðŸ“Š Today: ${todayTrades.length} trades | ${wins}W ${losses}L | Net: ${sign}${netPts.toFixed(2)} pts`;
    const tradeLines = todayTrades.map(t => {
      const ps = (t.pnl >= 0 ? "+" : "") + t.pnl;
      return `  ${t.direction} ${t.ticker} ${t.entry}â†’${t.exit} ${t.result} (${ps} pts)`;
    });
    const tradeCtx = loadTodayContext();
    const exitWarnings = checkEmotionalState(tradeCtx);
    const stateLines = exitWarnings.length > 0
      ? ["Emotional state: " + exitWarnings.map(w => `${w.emoji} ${w.message}`).join(" | ")]
      : ["Emotional state: âœ… Clear"];

    const replay = todaySummary();
    const replayLines = replay
      ? ['\nðŸ“Š Signal log: ' + replay.total + ' signals â€” ' +
  ...
  if (message.trim() === "/reset") {
    ...
    return res.json({ reply: "âœ… Daily reset complete. Paste /dubz and /heatmap to start." });
  }
  ...
  if (message.trim() === "/status") {
    const today = new Date().toISOString().split('T')[0];
    const statusLines = [];
    statusLines.push("LUKE ONLINE âœ…");
    statusLines.push("");
    ...
    let _cd = null;
    if (_mkt.open) {
      statusLines.push(`ðŸ• Market: OPEN â€” ${_tt.message}`);
    } else {
      const _mins = minsUntilOpen();
      const _h = Math.floor(_mins / 60);
      const _m = _mins % 60;
      _cd = _h > 0 ? `${_h}h ${_m}m` : `${_m}m`;
      statusLines.push(`ðŸ• Market: CLOSED â€” Opens in ${_cd}`);
    }

    const workflowStatus = getPhase2WorkflowLoadStatus();
    if (workflowStatus.dubzLoaded || workflowStatus.bobbyLoaded) {
      statusLines.push(`Levels: âœ… loaded (${workflowStatus.dubzCount} Dubz, ${workflowStatus.bobbyCount} Bobby mentions)`);
    } else {
      statusLines.push("Levels: âŒ not loaded");
    }
    ...
    if (!_mkt.open) {
      statusLines.push(`State: ðŸ”µ CLOSED â€” Market opens in ${_cd}`);
    } else {
      ...
      if (stHard) {
        statusLines.push(`State: ðŸ”´ RED â€” ${stHard.message}`);
      } else if (stSoft.length > 0) {
        statusLines.push(`State: ðŸŸ¡ YELLOW â€” ${stSoft.map(w => w.message).join(" | ")}`);
      } else {
        statusLines.push("State: ðŸŸ¢ GREEN");
      }
    }

    try {
      const regime = getSiennaRegime();
      statusLines.push(`Regime: ðŸŒ¡ï¸ ${regime.regime} â€” ${regime.reason}`);
    } catch {}
    ...
    try {
      ...
      if (fs.existsSync(LAST_SIGNAL_FILE)) {
        ...
        if (sig.date === today) {
          statusLines.push(`Last signal: ${sig.time} ${sig.ticker} â€” ${sig.verdict}`);
        } else {
  ...
  if (message.trim() === "/trading-mode") {
    ...
    exec('python -c "import win32gui"', (err) => {
      if (err) {
        broadcast({ type: 'notification', message: 'âš ï¸ trading-mode: pywin32 not installed. Run: pip install pywin32 --break-system-packages' });
        return;
      }
      exec('python "' + scriptPath + '"', (err2, stdout, stderr) => {
        if (err2) {
          ...
          broadcast({ type: 'notification', message: 'âš ï¸ trading-mode failed: ' + detail });
        }
      });
    });
    return res.json({ reply: 'ðŸ–¥ï¸ Layout launching â€” ximes top-left, Bobby bottom-left, Tradovate top-right, Luke bottom-right. Takes ~20s. Check top bar for errors.' });
  }

  // â”€â”€ SATY ATR LEVELS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if (message.startsWith('/saty ')) {
    ...
    if (!satyParsed.valid) {
      return res.json({ reply: `âŒ ${satyParsed.error || 'parse failed'}\n\nFormat: paste 13 levels highestâ†’lowest\n/saty 5920 5910 5900 5890 5880 5870 5860 5850 5840 5830 5820 5810 5800` });
    }
    saveSatyLevels(satyParsed);
    const savedSaty = loadSatyLevels();
    await appendSatyToMemory(savedSaty);
    return res.json({ reply: `ðŸ“ Saty levels saved:\n\n${buildStatusSummary(savedSaty)}\n\nWill appear in /alert as entry refinement.` });
  }

  // â”€â”€ DUBZ LEVELS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if (message.startsWith('/mancini ')) {
    ...
    if (parsed.levels.length === 0 && parsed.chop_zones.length === 0) {
      return res.json({ reply: `Ã¢ÂÅ’ Mancini parse failed.\n${parsed.parse_errors.join('\n')}` });
    }
    ...
    const lines = [`Ã°Å¸â€œËœ Mancini levels saved (${parsed.instrument} ${parsed.date})`];
  ...
  if (message.startsWith('/dubz') && (message.startsWith('/dubz ') || res._dubzImage)) {
      ...
      if (now - lastDubzVisionCallMs < VISION_RATE_LIMIT_MS) {
        ...
        try {
          ...
          if (imgResult?.parse_status === 'failed') {
            visionFailed = true;
            visionNote = `\nâŒ Vision error: ${imgResult.error}`;
          } else if (imgResult) {
    ...
    const conflictLine = newState.conflicts.length > 0
      ? `\nâš ï¸ ${newState.conflicts.length} conflict(s) detected â€” run /dubz to review`
      : '';
    ...
    if (newState.carry_forward_failed) {
      return res.json({ reply: `âš ï¸ Carry-forward requested but no prior Dubz state was found. Paste today's levels first.` });
    }

    const mixedSuccess = visionFailed && textResult;
    const dubzPrefix   = mixedSuccess ? 'âš ï¸ Partial parse â€” text levels captured, image vision failed\n' : '';
    const dubzTailNote = mixedSuccess ? visionNote : '';
    const dubzInline   = mixedSuccess ? '' : visionNote;
    // Suppress parse_errors redundancy in mixed-success: dubzTailNote already carries the âŒ detail
    const dubzErrLine  = mixedSuccess ? '' : errLine;

    return res.json({
      reply: `${dubzPrefix}ðŸ“Š Dubz levels updated${dubzInline}.\n${instrLines}${conflictLine}${dubzErrLine}${dubzTailNote}`,
    });
  }

  // â”€â”€ /verdict â€” confluence verdict across Level Memory â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  //
  // /verdict           â†’ NQ + ES + SPY, top 5 levels each by grade
  // /verdict ES        â†’ ES (+ SPX-equivalent) levels, top 5
  // /verdict NQ all    â†’ NQ (+ QQQ-equivalent) levels, no cap
  //
  ...
  if (message.trim() === '/verdict' || message.startsWith('/verdict ')) {
    ...
    // Get live prices for staleness grounding â€” fail-open if unavailable (C7)
    let livePrice = null;
    try {
      livePrice = await getLivePrice();
    } catch { /* no-op â€” staleness flags will be absent */ }
  ...
  if (message.startsWith('/backtest ')) {
    ...
    if (bars === null || bars.length === 0) {
      return res.json({ reply: `No data for ${instrument} on ${date}. Drop CSV at data/historical/${date}/${instrument}_1m.csv` });
    }
    ...
    const gradeLines = ['A', 'B', 'C', 'D', 'F']
      ...
      .map(grade => {
        ...
        return `${grade}: ${rows.length} lvls, ${(gradeRate * 100).toFixed(0)}% hold`;
      });
  ...
  if (message.startsWith('/entries ')) {
    ...
    const top = ranked.slice(0, 5).map(item => {
      const replayBars = null;
      const replay = replayBars ? replayLevelAgainstPriceAction(item.record, replayBars, {}) : null;
