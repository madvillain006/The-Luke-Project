...
function writeActiveTrade(signal, bracket) {
  ...
  try { fs.writeFileSync(ACTIVE_TRADE_FILE, JSON.stringify(entry, null, 2)); } catch {}
}
...
async function handleSlashCommand(message, res) {
  ...
  if (message.trim() === "/trading-mode") {
    ...
    const layoutScript = path.join(JARVIS_ROOT, "scripts", "trading-layout.py");
    try {
      execSync(`python "${layoutScript}"`, { timeout: 8000 });
    } catch (err) {
      log("layout", { event: "trading-mode script error", error: err.message });
    }
    return res.json({ reply: "📐 Trading mode active. Layout ready. Type /status when set." });
  }
