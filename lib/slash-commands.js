...
const DUBZ_FILE          = path.join(DATA_DIR, "dubz-levels.json");
...
async function handleSlashCommand(message, res) {
      ...
      if (now - lastVisionCallMs < VISION_RATE_LIMIT_MS) {
        ...
        try {
          ...
          if (visionResult && visionResult.parse_status === 'failed') {
            visionNote = ` (vision error: ${visionResult.error})`;
            ...
            visionNote = ' (vision returned no data)';
          }
        } catch (err) { visionNote = ` (vision exception: ${err.message})`; }
      }
    }
  ...
  if (message.startsWith('/dubz') && (message.startsWith('/dubz ') || res._dubzImage)) {
    ...
    let textResult = null;
    if (rawText) textResult = parseDubzText(rawText);
      ...
      if (now - lastDubzVisionCallMs < VISION_RATE_LIMIT_MS) {
        ...
        try {
          ...
          if (imgResult?.parse_status === 'failed') {
            visionNote = ` (vision error: ${imgResult.error})`;
          } else if (imgResult) {
            visionNote = ` + vision parsed (${imgResult.instrument || '?'}: ${(imgResult.levels || []).length} levels)`;
          }
        } catch (err) {
          visionNote = ` (vision exception: ${err.message})`;
        }
      }
    }
    ...
    return res.json({
      reply: `📊 Dubz levels updated${visionNote}.\n${instrLines}${conflictLine}${errLine}`,
    });
  }
