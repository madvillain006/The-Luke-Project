...
function isTradingMessage(message) {
  ...
  const STRONG_TERMS = /\b(pine|katbot|kaprik0rn3|barrysanders329|jefe|wyckoff|heatmap|strike|expiry|premium|mnq|mes|spx|ticker)\b/i;
...
function loadTradingContextPacks() {
  try {
    if (!fs.existsSync(TRADING_CONTEXT_DIR)) return [];
    return fs.readdirSync(TRADING_CONTEXT_DIR)
      .filter(f => f.endsWith(".json"))
      .sort()
      .map(file => {
        try {
          const full = path.join(TRADING_CONTEXT_DIR, file);
          const pack = JSON.parse(fs.readFileSync(full, "utf8"));
          return {
            id: pack.id || file.replace(/\.json$/i, ""),
            title: pack.title || pack.id || file,
            applies_to: pack.applies_to || [],
            summary: pack.bot_context_summary || [],
            rules: pack.trading_rules || [],
          };
        } catch {
          return null;
        }
...
function buildSystemPrompt(state, message) {
  ...
  const signalBlock = signals.length > 0
    ...
    : "";
  const tradingContextBlock = message && isTradingMessage(message) ? buildTradingContextBlock() : "";
  ...
  const lukeBaseline = "\nLUKE: Personal-care logging is retired. Do not suggest medication logging, feeding logs, or care reminders.\n";
...
1. MARKET DATA PROVIDER LAYER
   - Use lib/market-data for ES, MES, NQ, MNQ, SPX, SPY, and QQQ.
   - Provider priority is Tradovate futures quotes when configured, then Polygon/Massive latest close for supported index/ETF symbols, then Yahoo chart data as a delayed fallback.
   - Every price must carry source, timestamp, stale/delayed flags, confidence, and provider attempts. If no provider returns a trusted price, report UNKNOWN and do not guess.
   - Latest close and weekend data are stale/reference data. Do not present them as live intraday futures prices.
   - ES/NQ are no longer derived from SPX/SPY/QQQ approximations in live paths.

2. SATY LEVELS (generated from previous close)
   - Provides: SPX daily-bar computed Saty levels (ATR extensions, ribbon
     classification) from previous close and ATR context.
   - Availability: system-owned. Do not ask Conor to load Saty before trading commands.
   - /saty remains a manual override/debug command, not a required prep step.

3. KATBOT/SPX ANALYST HEATMAP
   - Provides: parsed heatmap panels from KatBot analyst image/text context. Includes
      ticker, strikes, premiums, and expiry context.
   - Available: when /heatmap is run with a fresh KatBot/SPX heatmap or analyst paste.
   - Uses getLivePrice() to ground strike plausibility. If live price is UNKNOWN,
...
4. RICHYDUBBZ SIGNALS (parse-dubz.js parser)
   ...
   - If live price is UNKNOWN: plausibility bounds are skipped, parsing continues.
   - If parse fails: report the parse error directly.
...
6. KATBOT / STAGE 2 RESEARCH CONTEXT
   - KatBot captures authorized Discord analyst posts for confluence analytics.
   - Stage 2 supports manual options/copy-trading paste analysis, KatBot confluence, Pine/Luke Watch context, and research-only Sybil/server notes.
   - Sybil is context only: regime, equity DD, event-stack, SpotGamma/HIRO/gamma walls, expression map, and risk notes. It is never a trigger source, never a live-order source, and never overrides KatBot analyst/Pine execution evidence.
   - Manual KatBot analyst/server paste capture writes ignored local Stage 2 context under data/research/katbot/manual-stage2 and must push back when a paste lacks ticker/contract, direction, entry, stop/invalidation, target/update, or timestamp.
   - Manual Sybil/server paste capture writes summarized context under data/research/sybil-katbot. It must not increase verified trade-call counts.
   - Image/OCR extraction remains disabled until free-provider vision routing is explicitly authorized.

7. MANCINI / EXTERNAL ANALYST FEEDS
...
- If a listed source returns null or fails, name the source and the failure
  ...
  Conversely, if a capability is NOT in this list, say so explicitly - do not invent a tool or claim a source you were not told about.
- Always state market price source and stale/delayed status when quoting it.
- Polygon/Massive latest-close and Yahoo fallback data may be delayed or stale. Do not present either as a real-time intraday futures price.
`;
  return "You are Luke - Conor's trading and build copilot. Not a generic assistant. Prioritize trading discipline, codebase help, and concise ops support.\n\n" +
    repoBlock + "\n\n" +
    "CORE CONTEXT:\n" +
    "- Personal-care logging is retired. Do not suggest medication logging, feeding logs, or care reminders.\n" +
    "- Trading matters most when the message is trading-related: ES futures, Apex discipline, Luke Watch Pine, Katbot/SPX heatmap, RichyDubz, Mancini, confluence, bracket, floor risk.\n" +
    ...
    "- Prefer /status, /verdict, /entries ES, /dubz, /heatmap, and /mancini as the live workflow. /saty is an override, not a required prep step.\n" +
    tradingContextBlock +
    signalBlock + "\n\n" +
    dataSourcesBlock + "\n" +
    "HOW YOU TALK:\n" +
...
module.exports = { buildSystemPrompt, loadDiscordSignals, loadTradingContextPacks, pruneDiscordHistory };
