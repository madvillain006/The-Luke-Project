...
function dedupeHeatmapSnapshots(events = []) {
  ...
  for (const event of events) {
    const key = snapshotHash(event);
    if (seen.has(key)) {
      const existing = out.find(item => item.snapshot_hash === key);
      if (existing) {
        const transports = new Set([
          ...(existing.transports || [existing.transport || 'unknown']),
          event.transport || 'unknown',
        ]);
        const duplicateIds = new Set([
          ...(existing.duplicate_ids || [existing.id].filter(Boolean)),
          event.id,
        ].filter(Boolean));
        existing.transports = [...transports].sort();
        existing.duplicate_ids = [...duplicateIds].sort();
        const existingTime = Date.parse(existing.available_at_et || existing.timestamp_et || 0);
        const eventTime = Date.parse(event.available_at_et || event.timestamp_et || 0);
        if (Number.isFinite(eventTime) && (!Number.isFinite(existingTime) || eventTime > existingTime)) {
          existing.id = event.id || existing.id;
          existing.transport = event.transport || existing.transport;
          existing.available_at_et = event.available_at_et || existing.available_at_et;
          existing.timestamp_et = event.timestamp_et || existing.timestamp_et;
        }
      }
      continue;
    }
    ...
    out.push({
      ...event,
      snapshot_hash: key,
      transports: [event.transport || 'unknown'],
      duplicate_ids: [event.id].filter(Boolean),
    });
  }
  return out;
}
