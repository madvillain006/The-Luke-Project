...
function levelToRow(event = {}, level = {}, timestamp) {
  ...
  return {
    ...
    transport: event.transport || enriched.transport || 'unknown',
    transports: [event.transport || enriched.transport || 'unknown'],
...
function buildLevelSnapshot({
  ...
  const activeHeatmapIds = new Set(heatmapLifecycle.active.map(event => event.id));
  const hasParsedHeatmap = heatmapLifecycle.active.length > 0;

  const allowed = keepLatestStructuralEvents(enriched.filter(event => {
    const family = event.source_family || normalizeSourceFamily(event.source, event);
    if (family === SOURCE_FAMILIES.HEATMAP_GEX) return activeHeatmapIds.has(event.id);
