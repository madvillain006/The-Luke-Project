...
const DEFAULT_LEVEL_RECLAIM_DIR = path.join(ROOT, 'tradingview', 'history', 'level-reclaim');
const DEFAULT_PRODUCTION_LEDGER_DIR = path.join(ROOT, 'tradingview', 'history', 'production-ledger');
const DEFAULT_SATY_REFERENCE = path.join(ROOT, 'tradingview', 'support', 'saty-reference', 'saty-atr-levels-source.pine');
const DEFAULT_BASE_PINE = path.join(DEFAULT_LEVEL_RECLAIM_DIR, 'luke-level-reclaim-watch.pine');
const DEFAULT_REALISTIC_PINE = path.join(DEFAULT_LEVEL_RECLAIM_DIR, 'luke-level-reclaim-watch-realistic-accounting.pine');
const DEFAULT_PRODUCTION_TEST_PINE = path.join(DEFAULT_PRODUCTION_LEDGER_DIR, 'luke-watch-production-test.pine');
const DEFAULT_READABLE_LEDGER_V5_PINE = path.join(DEFAULT_PRODUCTION_LEDGER_DIR, 'luke-watch-production-test-readable-ledger-v4-es-ledger-qol.pine');
const DEFAULT_SIMULATION_STRATEGY_PINE = path.join(DEFAULT_PRODUCTION_LEDGER_DIR, 'luke-watch-production-test-simulation.strategy.pine');
const DEFAULT_HARDMODE_PINE = path.join(DEFAULT_LEVEL_RECLAIM_DIR, 'luke-level-reclaim-watch-hardmode.strategy.pine');
...
function extractManciniLogPosts(rawText, { latestDate = null } = {}) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    let j = i + 1;
    if (/^Adam_Mankini$/i.test(line)) {
      while (j < lines.length && !lines[j].trim()) j += 1;
      commentLine = `${line} ${lines[j]?.trim() || ''}`;
      j += 1;
    }
    if (!/Adam_Mankini\s+commented/i.test(commentLine)) continue;
    const body = [];
    while (j < lines.length) {
      const trimmed = lines[j].trim();
    ...
    if (MANCINI_KEYWORD_RE.test(text)) {
      posts.push({
        text,
        transport: 'reddit',
        relative_date: commentLine.match(/commented\s+(.+?ago)/i)?.[1] || null,
        date_hint: inferRelativeDate(commentLine, latestDate),
      });
    }
  }
...
function extractManciniFallbackLevels(text) {
  ...
  const rangeRe = /\b(\d{4,5})-(\d{2,3})(?!\d)\b/g;
  let range;
...
function normalizeManciniRecord({
  ...
  return {
    ...
    freshness: {
      status: date >= activeCutoff ? 'current' : 'perpetual',
      date,
    },
    timestamp_et: timestamp,
    active: role !== 'chop_or_veto',
...
function extractManciniLevelsFromLog({ filePath, latestDate, now = new Date(), lookbackDays = 4 } = {}) {
...
function uniqueSortedPrices(levels, limit = Infinity) {
  const seen = new Set();
  const values = [];
  for (const level of levels || []) {
...
function writeTradingViewArtifacts({
  ...
  readableLedgerV5PinePath = DEFAULT_READABLE_LEDGER_V5_PINE,
  ...
  const generatedReadableLedgerV5Path = path.join(artifactDir, 'luke-watch-production-test-readable-ledger-v5-full-saty-ribbon.generated.pine');
  ...
  if (fs.existsSync(readableLedgerV5PinePath)) {
    const readableLedgerV5 = fs.readFileSync(readableLedgerV5PinePath, 'utf8');
    fs.writeFileSync(generatedReadableLedgerV5Path, renderGeneratedPine(readableLedgerV5, exportData.pine_inputs), 'utf8');
  }
  ...
  const pineFilesSummary = {
    ...
    files: [
      ...
      {
        path: path.relative(rootDir, readableLedgerV5PinePath),
        classification: 'Luke chart-facing readable ledger v5 - full Saty ribbon and perpetual Mancini levels',
        declaration: 'indicator',
        generated_artifact: fs.existsSync(generatedReadableLedgerV5Path) ? generatedReadableLedgerV5Path : null,
        order_logic: false,
        alertconditions: true,
      },
      {
      ...
      {
        path: path.relative(rootDir, DEFAULT_SATY_REFERENCE),
  ...
  const summary = {
    ...
    artifacts: {
      ...
      generated_readable_ledger_v5_indicator: fs.existsSync(generatedReadableLedgerV5Path) ? generatedReadableLedgerV5Path : null,
