...
namespace NinjaTrader.NinjaScript.Strategies
{
    ...
    public class LukeNativeShadowStrategy : Strategy
    {
        ...
        [NinjaScriptProperty]
        [Display(Name = "Central Mancini event CSV path", GroupName = "Luke Native", Order = 3)]
        public string ManciniEventCsvPath { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Telemetry file path", GroupName = "Luke Native", Order = 4)]
        public string TelemetryFilePath { get; set; }

        [NinjaScriptProperty]
        [Range(1, 3600)]
        [Display(Name = "Level refresh seconds", GroupName = "Luke Native", Order = 5)]
        public int LevelRefreshSeconds { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show parity ledger overlay", GroupName = "Luke Native", Order = 6)]
        public bool ShowParityLedgerOverlay { get; set; }

        [NinjaScriptProperty]
        [Range(3, 20)]
        [Display(Name = "Ledger overlay rows", GroupName = "Luke Native", Order = 7)]
        public int LedgerOverlayRows { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Pine bridge events path", GroupName = "Luke Native", Order = 8)]
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                Description = "Ninja-native port of the Luke v4 reclaim/bridge signal engine. Exact-account guarded modes can submit limit orders.";
                ...
                ManciniEventCsvPath = @"C:\Users\conor\luke\artifacts\research\mancini-context-protocol\events.csv";
                ...
                AllowedAccountName = "APEX752020000018";
                NativeQuantity = 2;
                MaxQuantity = 2;
        ...
        private void LoadLevelsIfNeeded()
        {
            ...
            LevelLoadResult centralLevels = TryLoadCentralManciniLevels(chartSessionDate);
            if (centralLevels != null)
            {
                ApplyLevelLoad(centralLevels, chartSessionDate);
                return;
            }
            ...
            LevelLoadResult levelFile = ParseTextLevelFile(text, targetSessionDate, LevelFilePath);
            ApplyLevelLoad(levelFile, chartSessionDate);
        }

        private void ApplyLevelLoad(LevelLoadResult result, DateTime chartSessionDate)
        {
            if (result == null)
                return;
            if (string.Equals(result.CacheText, lastLevelFileText, StringComparison.Ordinal)
                && chartSessionDate == lastLevelSessionDate)
                return;

            rawLevels.Clear();
            rawLevels.AddRange(result.Levels);
            lastLevelFileText = result.CacheText;
            lastLevelSessionDate = chartSessionDate;

            WriteTelemetry("LEVELS_LOADED", null, "count=" + rawLevels.Count.ToString(CultureInfo.InvariantCulture)
                + " raw_count=" + result.RawMatches.ToString(CultureInfo.InvariantCulture)
                + " rejected_count=" + result.RejectedMatches.ToString(CultureInfo.InvariantCulture)
                + " target_session=" + (result.TargetSessionDate == DateTime.MinValue ? "" : result.TargetSessionDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture))
                + " chart_session=" + (chartSessionDate == DateTime.MinValue ? "" : chartSessionDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture))
                + " bytes=" + result.Bytes.ToString(CultureInfo.InvariantCulture)
                + " has_sections=" + (result.HasSections ? "true" : "false")
                + " source=" + result.Source
                + " path=" + result.Path);
        }

        private LevelLoadResult ParseTextLevelFile(string text, DateTime targetSessionDate, string sourcePath)
        {
            string[] lines = (text ?? string.Empty).Split(new[] { "\r\n", "\n" }, StringSplitOptions.None);
            bool hasSections = Regex.IsMatch(text ?? string.Empty, @"(?im)^\s*(trade|mancini|major|focus_long|target_only|read_reaction|caution|trigger|reclaim)\s*:");
            List<LevelInput> levels = new List<LevelInput>();
            int rawMatches = 0;
            ...
            for (int lineIndex = 0; lineIndex < lines.Length; lineIndex++)
            {
                ...
                foreach (Match match in Regex.Matches(line, @"[-+]?\d+(?:\.\d+)?"))
                {
                    ...
                    InsertSorted(levels, new LevelInput(RoundPrice(value), "mancini"));
                }
            }
            return new LevelLoadResult(levels, text ?? string.Empty, targetSessionDate, rawMatches, rejectedMatches, (text ?? string.Empty).Length, hasSections, "level_file", sourcePath);
        }

        private LevelLoadResult TryLoadCentralManciniLevels(DateTime chartSessionDate)
        {
            if (chartSessionDate == DateTime.MinValue || string.IsNullOrWhiteSpace(ManciniEventCsvPath))
                return null;

            string path = ManciniEventCsvPath.Trim();
            string text;
            try
            {
                if (!File.Exists(path))
            ...
            {
                WriteTelemetry("LEVEL_LOAD_ERROR", null, "source=central_events_csv path=" + path + " error=" + ex.Message);
                return null;
            }
            ...
            {
                string key = (headers[i] ?? string.Empty).Trim();
                if (key.Length > 0 && !headerIndex.ContainsKey(key))
                    headerIndex[key] = i;
            }

            if (!headerIndex.ContainsKey("plan_date") || !headerIndex.ContainsKey("price"))
                return null;

            string sessionKey = chartSessionDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
            List<LevelInput> levels = new List<LevelInput>();
            ...
            for (int lineIndex = 1; lineIndex < lines.Length; lineIndex++)
            {
                ...
                if (!string.Equals(CsvValue(columns, headerIndex, "plan_date"), sessionKey, StringComparison.Ordinal))
                    continue;
                if (!string.Equals(CsvValue(columns, headerIndex, "direction"), "support", StringComparison.OrdinalIgnoreCase))
                    continue;
                if (!CsvTruthy(CsvValue(columns, headerIndex, "long_eligible")))
                    continue;

                rawMatches++;
                double price;
                if (!double.TryParse(CsvValue(columns, headerIndex, "price"), NumberStyles.Float, CultureInfo.InvariantCulture, out price)
                    || !IsValidExternalEsLevel(price))
                {
                    rejectedMatches++;
                    continue;
                }

                InsertSorted(levels, new LevelInput(RoundPrice(price), "mancini"));
            ...
            string cacheText = "central_events_csv|" + path + "|" + sessionKey + Environment.NewLine + cache.ToString();
            return new LevelLoadResult(levels, cacheText, chartSessionDate.Date, rawMatches, rejectedMatches, text.Length, false, "central_events_csv", path);
        }
        ...
        private static string CsvValue(List<string> columns, Dictionary<string, int> headerIndex, string key)
        {
            int index;
            if (columns == null || headerIndex == null || !headerIndex.TryGetValue(key, out index) || index < 0 || index >= columns.Count)
                return string.Empty;
            return (columns[index] ?? string.Empty).Trim();
        }

        private static bool CsvTruthy(string value)
        {
        ...
        private class LevelLoadResult
        {
            public LevelLoadResult(List<LevelInput> levels, string cacheText, DateTime targetSessionDate, int rawMatches, int rejectedMatches, int bytes, bool hasSections, string source, string path)
            {
                Levels = levels ?? new List<LevelInput>();
            ...
            public List<LevelInput> Levels;
            public string CacheText;
            public DateTime TargetSessionDate;
            public int RawMatches;
            public int RejectedMatches;
            public int Bytes;
